---
name: trend
description: Scan the web for trending news across Kritano's 6 pillars (Accessibility, SEO, Security, Performance, Content Quality, AI Readiness), then generate ready-to-post social media content. Also supports --package mode for evergreen content packages (blog + social + visuals + video) without trend scanning.
user-invocable: true
...
`/trend` or `/trend seo` or `/trend accessibility`
...
**Otherwise:** If a specific pillar is named (e.g. "seo", "accessibility"), focus the scan on that pillar. If blank or "all", scan all 6 pillars.
