...
server {
    ...
    # Blog SSR — served by Express with full HTML (no JS needed for indexing)
    # Express sets its own CSP for SSR pages; hide the parent server-block CSP
    # so the browser only sees one Content-Security-Policy header.
    location /blog {
        proxy_pass http://127.0.0.1:3001;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
    ...
    # Sitemap (dynamically generated by Express)
