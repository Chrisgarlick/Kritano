---
title: "Integrating Accessibility Testing into Your CI/CD Pipeline with Kritano"
slug: "accessibility-testing-cicd-pipeline-kritano"
date: "2026-03-18"
author: "Chris Garlick"
description: "Catch accessibility regressions before they reach production. Here's how to integrate Kritano's API into GitHub Actions and GitLab CI with real examples."
...
# Integrating Accessibility Testing into Your CI/CD Pipeline with Kritano
...
Kritano's API makes this straightforward. You can trigger an audit, wait for results, and fail the build if the accessibility score drops below a threshold you define — all within a standard GitHub Actions or GitLab CI workflow. Let me walk you through exactly how to set it up.
...
Head to your Kritano dashboard and navigate to **Settings > API Keys**. Create a new key with the `audits:write` and `audits:read` scopes.

Your key will look something like `kt_live_xxxxxxxxxxxxxxxx`. Store it as a secret in your CI/CD platform — never commit it to your repository.

**In GitHub:** Go to your repository Settings > Secrets and variables > Actions, and add a new secret called `KRITANO_API_KEY`.

**In GitLab:** Go to Settings > CI/CD > Variables and add `KRITANO_API_KEY` as a masked variable.

## Step 2: Understand the API Flow

The Kritano API uses an asynchronous audit model:
...
The API authenticates via the `X-API-Key` header or `Authorization: Bearer kt_live_xxx`.

## Step 3: GitHub Actions Example

Here's a complete workflow that runs a Kritano accessibility audit on every pull request and fails the build if the score drops below your threshold:
...
jobs:
  accessibility:
    ...
    steps:
      - name: Trigger Kritano Audit
        id: trigger
        run: |
          RESPONSE=$(curl -s -X POST \
            https://kritano.com/api/v1/audits \
            -H "X-API-Key: ${{ secrets.KRITANO_API_KEY }}" \
      ...
      - name: Wait for Audit Completion
        ...
        run: |
          ...
          for i in $(seq 1 30); do
            RESULT=$(curl -s \
              https://kritano.com/api/v1/audits/$AUDIT_ID \
              -H "X-API-Key: ${{ secrets.KRITANO_API_KEY }}")
...
**What this does:** On every PR to main, it triggers a Kritano audit against your staging URL, waits for it to complete (checking every 20 seconds for up to 10 minutes), and fails the build if the accessibility score is below 70.
...
**Don't audit localhost.** The Kritano API needs a publicly accessible URL. Point it at your staging deployment, not your local dev server.
...
If you haven't got a Kritano API key yet, head to kritano.com and set one up. The API is available on all paid tiers, and you can be running accessibility checks in your pipeline by the end of today.
...
- Link "Kritano API" to the API docs page
-->
