...
exports.adminColdProspectsRouter = void 0;
...
exports.adminColdProspectsRouter = router;
/**
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await (0, cold_prospect_admin_service_js_1.getProspects)(filters);
        res.json({
...
router.get('/stats', async (_req, res) => {
    try {
        const stats = await (0, cold_prospect_admin_service_js_1.getStats)();
        res.json({ stats });
    }
    catch (error) {
...
router.get('/daily-stats', async (req, res) => {
    try {
        ...
        const stats = await (0, cold_prospect_admin_service_js_1.getDailyStats)(days);
        res.json({ stats });
    }
    catch (error) {
...
router.get('/tlds', async (_req, res) => {
    try {
        const tlds = await (0, cold_prospect_admin_service_js_1.getDistinctTlds)();
        res.json({ tlds });
    }
    catch (error) {
...
router.get('/settings', async (_req, res) => {
    try {
        const settings = await (0, cold_prospect_admin_service_js_1.getPipelineSettings)();
        res.json({ settings });
    }
    catch (error) {
...
const updateSettingsSchema = zod_1.z.object({
    targetTlds: zod_1.z.array(zod_1.z.string()).optional(),
    excludedKeywords: zod_1.z.array(zod_1.z.string()).optional(),
    minQualityScore: zod_1.z.number().min(0).max(100).optional(),
    dailyCheckLimit: zod_1.z.number().min(10).max(10000).optional(),
    dailyEmailLimit: zod_1.z.number().min(1).max(1000).optional(),
    autoOutreachEnabled: zod_1.z.boolean().optional(),
});
/**
 * PUT /api/admin/cold-prospects/settings
 * Update pipeline settings
 */
router.put('/settings', (0, validate_middleware_js_1.validateBody)(updateSettingsSchema), async (req, res) => {
    try {
        await (0, cold_prospect_admin_service_js_1.updatePipelineSettings)(req.body);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'update_cold_prospect_settings', 'system', 'cold_prospects', req.body, req);
        const settings = await (0, cold_prospect_admin_service_js_1.getPipelineSettings)();
        res.json({ settings });
    }
    catch (error) {
...
router.get('/outreach-stats', async (_req, res) => {
    try {
        const stats = await (0, outreach_service_js_1.getOutreachStats)();
        res.json({ stats });
    }
    catch (error) {
...
router.get('/sends', async (req, res) => {
    try {
        ...
        const result = await (0, outreach_service_js_1.getSendHistory)(page, limit);
        res.json({
            sends: result.sends,
            pagination: {
                page,
...
router.post('/trigger-outreach', async (req, res) => {
    try {
        ...
        const queued = await (0, outreach_service_js_1.queueOutreachBatch)(limit);
        const sent = await (0, outreach_service_js_1.processOutreachQueue)(limit);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'trigger_cold_outreach', 'system', 'cold_prospects', { queued: queued.queued, sent: sent.sent, failed: sent.failed }, req);
 ...
 */
router.post('/pause-outreach', async (req, res) => {
    try {
        const current = await (0, system_settings_service_js_1.getSetting)('trigger_auto_send_cold_outreach');
        const newValue = !(current === true || current === 'true');
        await (0, system_settings_service_js_1.setSetting)('trigger_auto_send_cold_outreach', newValue, req.admin.id);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, newValue ? 'enable_cold_outreach' : 'disable_cold_outreach', 'system', 'cold_prospects', { enabled: newValue }, req);
...
router.get('/:id', async (req, res) => {
    try {
        const prospect = await (0, cold_prospect_admin_service_js_1.getProspect)(req.params.id);
        if (!prospect) {
...
router.delete('/:id', async (req, res) => {
    try {
        const reason = req.query.reason || 'Admin excluded';
        await (0, cold_prospect_admin_service_js_1.excludeProspect)(req.params.id, reason);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'exclude_cold_prospect', 'cold_prospect', req.params.id, { reason }, req);
...
router.post('/:id/retry', async (req, res) => {
    try {
        await (0, cold_prospect_admin_service_js_1.retryProspect)(req.params.id);
        res.json({ success: true });
    }
    catch (error) {
...
const bulkExcludeSchema = zod_1.z.object({
 ...
 */
router.post('/bulk-exclude', (0, validate_middleware_js_1.validateBody)(bulkExcludeSchema), async (req, res) => {
    try {
        const { ids, reason } = req.body;
        const excluded = await (0, cold_prospect_admin_service_js_1.bulkExclude)(ids, reason);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'bulk_exclude_cold_prospects', 'cold_prospect', 'bulk', { count: excluded, reason }, req);
        res.json({ excluded });
    }
    catch (error) {
...
router.post('/import', async (req, res) => {
    try {
        ...
        const result = await (0, nrd_feed_service_js_1.importFromCsv)(csv);
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'import_cold_prospects', 'cold_prospect', 'import', result, req);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.post('/import-json', (0, express_1.json)({ limit: '10mb' }), async (req, res) => {
    try {
        ...
        for (const p of prospects) {
            ...
            try {
                const result = await index_js_1.pool.query(`INSERT INTO cold_prospects (
        ...
        await (0, admin_middleware_js_1.logAdminActivity)(req.admin.id, 'import_cold_prospects_json', 'cold_prospect', 'import', result, req);
        res.json(result);
    }
    catch (error) {
...
//# sourceMappingURL=cold-prospects.js.map