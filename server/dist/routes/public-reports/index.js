...
exports.publicReportsRouter = void 0;
...
exports.publicReportsRouter = router;
/**
 ...
 */
router.post('/audits/:id/share', auth_middleware_js_1.authenticate, async (req, res) => {
    try {
        ...
        const auditResult = await index_js_1.pool.query(`SELECT aj.id, aj.site_id, aj.user_id, aj.share_token, aj.share_expires_at
       FROM audit_jobs aj
       WHERE aj.id = $1`, [auditId]);
        ...
        if (!hasAccess && audit.site_id) {
            const memberResult = await index_js_1.pool.query(`SELECT 1 FROM site_members WHERE site_id = $1 AND user_id = $2`, [audit.site_id, userId]);
        ...
        await index_js_1.pool.query(`UPDATE audit_jobs SET share_token = $1, share_expires_at = $2 WHERE id = $3`, [shareToken, expiresAt.toISOString(), auditId]);
 ...
 */
router.delete('/audits/:id/share', auth_middleware_js_1.authenticate, async (req, res) => {
    try {
        const auditId = req.params.id;
        const userId = req.user.id;
        // Verify audit exists and user has access
        const auditResult = await index_js_1.pool.query(`SELECT aj.id, aj.site_id, aj.user_id
       FROM audit_jobs aj
       WHERE aj.id = $1`, [auditId]);
        if (auditResult.rows.length === 0) {
            res.status(404).json({ error: 'Audit not found' });
        ...
        await index_js_1.pool.query(`UPDATE audit_jobs SET share_token = NULL, share_expires_at = NULL WHERE id = $1`, [auditId]);
 ...
 */
router.get('/public/reports/:token', async (req, res) => {
    try {
       ...
       WHERE aj.share_token = $1
         AND aj.share_expires_at > NOW()`, [token]);
        ...
        const findingsSummary = await index_js_1.pool.query(`SELECT severity, COUNT(DISTINCT rule_id) as count
       FROM audit_findings
       ...
       ORDER BY
        ...
        const categorySummary = await index_js_1.pool.query(`SELECT category, COUNT(DISTINCT rule_id) as count
       FROM audit_findings
       WHERE audit_job_id = $1 AND status != 'dismissed'
       GROUP BY category`, [audit.id]);
        res.json({
            audit: {
            ...
            findingsSummary: findingsSummary.rows.reduce((acc, row) => {
                acc[row.severity] = parseInt(row.count, 10);
                return acc;
            }, {}),
            categorySummary: categorySummary.rows.reduce((acc, row) => {
