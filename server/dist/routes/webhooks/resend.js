...
exports.resendWebhookRouter = router;
...
function verifyWebhookSignature(payload, headers) {
    ...
    for (const sig of sigParts) {
        ...
        try {
            if (crypto_1.default.timingSafeEqual(Buffer.from(expectedSignature), Buffer.from(sigValue))) {
                return true;
            }
        }
        catch {
...
router.post('/', async (req, res) => {
    try {
        ...
        const sendResult = await index_js_1.pool.query(`SELECT id, campaign_id, status FROM email_sends WHERE resend_message_id = $1`, [emailId]);
        ...
        try {
            await index_js_1.pool.query(`INSERT INTO email_events (email_send_id, campaign_id, event_type, payload, resend_event_id)
         VALUES ($1, $2, $3, $4, $5)`, [send.id, send.campaign_id, eventType, JSON.stringify(eventData), resendEventId]);
        }
        catch (err) {
            ...
            if (newOrder > currentOrder) {
                const timestampField = newStatus === 'opened' ? 'opened_at'
                    : newStatus === 'clicked' ? 'clicked_at'
                        : null;
                if (timestampField) {
                    await index_js_1.pool.query(`UPDATE email_sends SET status = $2, ${timestampField} = NOW() WHERE id = $1`, [send.id, newStatus]);
                }
                else {
                    await index_js_1.pool.query(`UPDATE email_sends SET status = $2 WHERE id = $1`, [send.id, newStatus]);
                }
            }
        }
        ...
        if (send.campaign_id && EVENT_TO_STAT[eventType]) {
            ...
            await (0, email_campaign_service_js_1.incrementCampaignStat)(send.campaign_id, statField);
        }
        res.json({ received: true });
    }
    catch (error) {
