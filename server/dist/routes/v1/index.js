...
const apiAuth_middleware_js_1 = require("../../middleware/apiAuth.middleware.js");
...
router.use(apiAuth_middleware_js_1.authenticateApiKey);
// Validation schemas
const createAuditSchema = zod_1.z.object({
    ...
    options: zod_1.z.object({
        ...
        checks: zod_1.z.array(zod_1.z.enum(['seo', 'accessibility', 'security', 'performance', 'content', 'file-extraction'])).optional(),
 ...
 * Create a new audit
 */
router.post('/audits', (0, apiAuth_middleware_js_1.requireScope)('audits:write'), (0, validate_middleware_js_1.validateBody)(createAuditSchema), async (req, res) => {
    try {
        const apiReq = req;
        ...
        const limits = apiKey_service_js_1.RATE_LIMIT_TIERS[tier];
 ...
 * List audits
 */
router.get('/audits', (0, apiAuth_middleware_js_1.requireScope)('audits:read'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
 ...
 * Get audit details
 */
router.get('/audits/:id', (0, apiAuth_middleware_js_1.requireScope)('audits:read'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
        const auditId = req.params.id;
        const result = await pool.query(`SELECT * FROM audit_jobs WHERE id = $1 AND user_id = $2`, [auditId, userId]);
 ...
 * Get audit findings
 */
router.get('/audits/:id/findings', (0, apiAuth_middleware_js_1.requireScope)('findings:read'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
        const auditId = req.params.id;
 ...
 * Cancel an audit
 */
router.post('/audits/:id/cancel', (0, apiAuth_middleware_js_1.requireScope)('audits:write'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
        const auditId = req.params.id;
 ...
 * Delete an audit
 */
router.delete('/audits/:id', (0, apiAuth_middleware_js_1.requireScope)('audits:write'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
        const auditId = req.params.id;
 ...
 * List user's sites
 */
router.get('/sites', (0, apiAuth_middleware_js_1.requireScope)('audits:read'), async (req, res) => {
    try {
        const apiReq = req;
        const userId = apiReq.apiUserId;
        ...
        const limitNum = Math.min(100, Math.max(1, parseInt(limit, 10) || 20));
        const offset = (pageNum - 1) * limitNum;
        const result = await pool.query(`SELECT id, domain, name, verified, created_at
         FROM sites
         WHERE COALESCE(owner_id, created_by) = $1
         ORDER BY created_at DESC
         LIMIT $2 OFFSET $3`, [userId, limitNum, offset]);
        const countResult = await pool.query(`SELECT COUNT(*) as count FROM sites WHERE COALESCE(owner_id, created_by) = $1`, [userId]);
        const total = parseInt(countResult.rows[0].count, 10);
        res.json({
