...
async function seed() {
    ...
    try {
        ...
        const passwordHash = await argon2_1.default.hash(SEED_PASSWORD, {
            type: argon2_1.default.argon2id,
            memoryCost: 65536,
            timeCost: 3,
            parallelism: 4,
        });
        const userResult = await pool.query(`INSERT INTO users (email, password_hash, first_name, last_name, email_verified, status, role, is_super_admin)
       ...
       ON CONFLICT (email) DO UPDATE SET
         ...
         email_verified = true,
         is_super_admin = true
       RETURNING id`, [SEED_EMAIL, passwordHash, SEED_FIRST_NAME, SEED_LAST_NAME]);
        const userId = userResult.rows[0].id;
        console.log(`  ✓ Admin user created (id: ${userId})`);
        // 2. Seed organization
        const orgSlug = `user-${userId.replace(/-/g, '')}`;
        await pool.query(`INSERT INTO organizations (name, slug, owner_id)
       VALUES ($1, $2, $3)
       ON CONFLICT DO NOTHING`, [`${SEED_FIRST_NAME}'s Workspace`, orgSlug, userId]);
        console.log(`  ✓ Organization created`);
        // 3. Seed free-tier subscription (if subscriptions table exists)
        try {
            await pool.query(`INSERT INTO subscriptions (user_id, tier, status)
        ...
        const owenHash = await argon2_1.default.hash(owenPassword, {
            type: argon2_1.default.argon2id,
            memoryCost: 65536,
            timeCost: 3,
            parallelism: 4,
        });
        const owenResult = await pool.query(`INSERT INTO users (email, password_hash, first_name, last_name, email_verified, status, role, beta_access)
       ...
       RETURNING id`, [owenHash]);
        ...
        await pool.query(`INSERT INTO organizations (name, slug, owner_id)
       VALUES ($1, $2, $3)
       ON CONFLICT DO NOTHING`, ["Owen's Workspace", owenSlug, owenId]);
        await pool.query(`INSERT INTO subscriptions (user_id, tier, status)
       VALUES ($1, 'free', 'active')
       ON CONFLICT DO NOTHING`, [owenId]);
        // 5. Initialize lead scores for seeded users
        await pool.query(`UPDATE users SET lead_score = 5, lead_status = 'new', lead_score_updated_at = NOW()
