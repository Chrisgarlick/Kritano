...
class AuditWorkerService {
     ...
     */
    async sendStallAlert(jobId, url, runningMins) {
        const html = `
    ...
    `;
        await email_service_js_1.emailService.sendGenericEmail('info@kritano.com', 'URGENT: Audit worker stalled', html);
            ...
            try {
                ...
                for (const [jobId, info] of this.jobStartTimes) {
                    if (!this.settledJobs.has(jobId) && !this.stallAlertSent.has(jobId) && (now - info.startedAt) > STALL_THRESHOLD_MS) {
                        ...
                        this.sendStallAlert(jobId, info.url, runningMins).catch(err => console.error('Failed to send stall alert email:', err));
                    }
                }
                ...
                for (const pageRow of crawledPages.rows) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            mobileResult = await this.auditCoordinator.analyzeMobilePage(mobileCrawlResult, mobilePage, auditConfig);
                        }
                        finally {
                            if (mobilePage) {
                                try {
     ...
     */
    static BOT_BLOCKING_DOMAINS = new Set([
        'linkedin.com',
        'www.linkedin.com',
        'uk.linkedin.com',
        'facebook.com',
