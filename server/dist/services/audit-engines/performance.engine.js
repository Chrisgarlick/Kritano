...
class PerformanceEngine {
    rules = [
        ...
        // Page size
        {
            ...
            check: (ctx) => {
                ...
                if (totalSize > THRESHOLDS.PAGE_SIZE_POOR) {
                    return this.createFinding('large-page', 'Very Large Page Size', 'serious', `Total page size: ${this.formatBytes(totalSize)} (target: <${this.formatBytes(THRESHOLDS.PAGE_SIZE_GOOD)})`, 'Reduce page weight by optimizing images, minifying code, and removing unused assets', totalSize, THRESHOLDS.PAGE_SIZE_GOOD);
                }
                else if (totalSize > THRESHOLDS.PAGE_SIZE_GOOD) {
                    return this.createFinding('large-page', 'Large Page Size', 'moderate', `Total page size: ${this.formatBytes(totalSize)} (target: <${this.formatBytes(THRESHOLDS.PAGE_SIZE_GOOD)})`, 'Consider reducing page weight', totalSize, THRESHOLDS.PAGE_SIZE_GOOD);
                }
                return null;
            },
        },
        // HTML size
        {
            ...
            check: (ctx) => {
                if (ctx.pageSizeBytes > THRESHOLDS.HTML_SIZE_POOR) {
                    return this.createFinding('large-html', 'Very Large HTML', 'serious', `HTML size: ${this.formatBytes(ctx.pageSizeBytes)} (target: <${this.formatBytes(THRESHOLDS.HTML_SIZE_GOOD)})`, 'Reduce HTML size by removing inline styles/scripts and unnecessary markup', ctx.pageSizeBytes, THRESHOLDS.HTML_SIZE_GOOD);
                }
                else if (ctx.pageSizeBytes > THRESHOLDS.HTML_SIZE_GOOD) {
                    return this.createFinding('large-html', 'Large HTML', 'moderate', `HTML size: ${this.formatBytes(ctx.pageSizeBytes)} (target: <${this.formatBytes(THRESHOLDS.HTML_SIZE_GOOD)})`, 'Consider reducing HTML document size', ctx.pageSizeBytes, THRESHOLDS.HTML_SIZE_GOOD);
                }
                return null;
            },
        },
        // Too many requests
        {
            ...
            check: (ctx) => {
                ...
                if (requestCount > THRESHOLDS.MAX_REQUESTS) {
                    return this.createFinding('too-many-requests', 'Too Many HTTP Requests', 'serious', `Page makes ${requestCount} requests (target: <${THRESHOLDS.MAX_REQUESTS})`, 'Reduce HTTP requests by combining files, using sprites, and lazy loading', requestCount, THRESHOLDS.MAX_REQUESTS);
                }
                else if (requestCount > THRESHOLDS.MAX_REQUESTS * 0.7) {
                    return this.createFinding('too-many-requests', 'Many HTTP Requests', 'moderate', `Page makes ${requestCount} requests (target: <${THRESHOLDS.MAX_REQUESTS})`, 'Consider reducing the number of HTTP requests', requestCount, THRESHOLDS.MAX_REQUESTS);
                }
                return null;
            },
        },
        // Too many scripts
        {
            ...
            check: (ctx) => {
                ...
                if (scripts.length > THRESHOLDS.MAX_SCRIPTS) {
                    return this.createFinding('too-many-scripts', 'Too Many Scripts', 'moderate', `Page loads ${scripts.length} scripts (target: <${THRESHOLDS.MAX_SCRIPTS})`, 'Bundle JavaScript files to reduce HTTP requests', scripts.length, THRESHOLDS.MAX_SCRIPTS);
                }
                return null;
            },
        },
        // Large JavaScript
        {
            ...
            check: (ctx) => {
                ...
                if (totalSize > THRESHOLDS.TOTAL_SCRIPTS_SIZE) {
                    return this.createFinding('large-javascript', 'Large JavaScript Payload', 'serious', `Total JS size: ${this.formatBytes(totalSize)} (target: <${this.formatBytes(THRESHOLDS.TOTAL_SCRIPTS_SIZE)})`, 'Minify JavaScript, implement code splitting, and remove unused code', totalSize, THRESHOLDS.TOTAL_SCRIPTS_SIZE);
                }
                return null;
            },
        },
        // Too many stylesheets
        {
            ...
            check: (ctx) => {
                ...
                if (styles.length > THRESHOLDS.MAX_STYLESHEETS) {
                    return this.createFinding('too-many-stylesheets', 'Too Many Stylesheets', 'minor', `Page loads ${styles.length} stylesheets (target: <${THRESHOLDS.MAX_STYLESHEETS})`, 'Combine CSS files to reduce HTTP requests', styles.length, THRESHOLDS.MAX_STYLESHEETS);
                }
                return null;
            },
        },
        // Large CSS
        {
            ...
            check: (ctx) => {
                ...
                if (totalSize > THRESHOLDS.TOTAL_STYLES_SIZE) {
                    return this.createFinding('large-css', 'Large CSS Payload', 'moderate', `Total CSS size: ${this.formatBytes(totalSize)} (target: <${this.formatBytes(THRESHOLDS.TOTAL_STYLES_SIZE)})`, 'Minify CSS, remove unused styles, and consider critical CSS extraction', totalSize, THRESHOLDS.TOTAL_STYLES_SIZE);
                }
                return null;
            },
        },
        // Too many fonts
        {
            ...
            check: (ctx) => {
                ...
                if (fonts.length > THRESHOLDS.MAX_FONTS) {
                    return this.createFinding('too-many-fonts', 'Too Many Web Fonts', 'minor', `Page loads ${fonts.length} fonts (target: <${THRESHOLDS.MAX_FONTS})`, 'Reduce font variants and consider system fonts', fonts.length, THRESHOLDS.MAX_FONTS);
                }
                return null;
            },
        },
            ...
            check: (ctx) => {
                ...
                ctx.$('img').each((_, el) => {
                    ...
                    if (src.startsWith('data:') || !src)
                        return;
                    if (!width || !height) {
                        findings.push(this.createFinding('images-without-dimensions', 'Image Without Dimensions', 'moderate', 'Image missing width/height attributes (causes layout shift)', 'Add explicit width and height attributes to prevent CLS', undefined, undefined, `img[src="${src.substring(0, 60)}"]`));
                    }
                });
                // Limit to avoid spam
        ...
        // Missing lazy loading
        {
            ...
            check: (ctx) => {
                ...
                if (totalImages > 5 && lazyLoadedCount < totalImages / 2) {
                    return this.createFinding('no-lazy-loading', 'Missing Lazy Loading', 'minor', `Only ${lazyLoadedCount}/${totalImages} images use lazy loading`, 'Add loading="lazy" to below-fold images', totalImages - lazyLoadedCount, undefined);
                }
                return null;
            },
        },
        // Missing text compression hint (checked via content-encoding)
        {
            ...
            check: (ctx) => {
                ...
                if (inlineScriptSize > 50 * 1024) {
                    findings.push(this.createFinding('large-inline-scripts', 'Large Inline Scripts', 'moderate', `Inline scripts total: ${this.formatBytes(inlineScriptSize)}`, 'Move large scripts to external files for better caching and compression', inlineScriptSize, 50 * 1024));
                }
                if (inlineStyleSize > 20 * 1024) {
                    findings.push(this.createFinding('large-inline-styles', 'Large Inline Styles', 'moderate', `Inline styles total: ${this.formatBytes(inlineStyleSize)}`, 'Move large styles to external files for better caching', inlineStyleSize, 20 * 1024));
                }
        ...
        // Document write usage
        {
            ...
            check: (ctx) => {
                if (ctx.html.includes('document.write(') || ctx.html.includes('document.write (')) {
                    return this.createFinding('document-write', 'Uses document.write()', 'moderate', 'Page uses document.write() which can block parsing', 'Replace document.write() with DOM manipulation methods');
                }
                return null;
            },
        },
        // Missing compression (#30)
        {
            ...
            check: (ctx) => {
                ...
                if (!encoding && ctx.pageSizeBytes > 1024) {
                    return this.createFinding('missing-compression', 'Missing Response Compression', 'moderate', `HTML response (${this.formatBytes(ctx.pageSizeBytes)}) is not compressed`, 'Enable gzip or brotli compression on your server to reduce transfer sizes by 60-80%', ctx.pageSizeBytes);
                }
                return null;
            },
        },
        // Image optimisation (#31)
        {
            ...
            check: (ctx) => {
                const findings = [];
                const largeImages = ctx.resources.filter(r => r.type === 'image' && r.size > THRESHOLDS.IMAGE_SIZE_POOR);
                for (const img of largeImages.slice(0, 5)) {
                    findings.push(this.createFinding('unoptimized-images', 'Large Image', 'moderate', `Image ${this.formatBytes(img.size)}: ${img.url.substring(0, 80)}`, 'Compress images using WebP/AVIF format and resize to appropriate dimensions', img.size, THRESHOLDS.IMAGE_SIZE_GOOD));
                }
                const totalImageSize = ctx.resources
                    .filter(r => r.type === 'image')
                    .reduce((sum, r) => sum + r.size, 0);
                if (totalImageSize > THRESHOLDS.TOTAL_IMAGES_SIZE) {
                    findings.push(this.createFinding('total-images-too-large', 'Total Image Payload Too Large', 'serious', `Total image payload: ${this.formatBytes(totalImageSize)} (target: <${this.formatBytes(THRESHOLDS.TOTAL_IMAGES_SIZE)})`, 'Optimize images using modern formats (WebP/AVIF), lazy loading, and appropriate sizing', totalImageSize, THRESHOLDS.TOTAL_IMAGES_SIZE));
                }
                return findings.length > 0 ? findings : null;
            },
        },
        // Missing responsive images (#32)
        {
            ...
            check: (ctx) => {
                ...
                if (totalContentImages > 3 && imagesWithoutSrcset > totalContentImages / 2) {
                    return this.createFinding('missing-srcset', 'Missing Responsive Images', 'minor', `${imagesWithoutSrcset}/${totalContentImages} images lack srcset attribute`, 'Add srcset and sizes attributes to serve appropriately sized images for different devices', imagesWithoutSrcset);
                }
                return null;
            },
        },
            ...
            check: (ctx) => {
                ...
                ctx.$('head link[rel="stylesheet"]').each((_, el) => {
                    ...
                    if (href && !href.startsWith('data:')) {
                        findings.push(this.createFinding('render-blocking-css', 'Render-Blocking CSS', 'moderate', `Stylesheet blocks rendering: ${href.substring(0, 60)}`, 'Inline critical CSS, defer non-critical CSS, or use media queries to reduce blocking', undefined, undefined, `link[href="${href.substring(0, 50)}"]`));
                    }
                });
                return findings.length > 0 ? findings.slice(0, 5) : null;
            },
        ...
        // Large DOM size (#34)
        {
            ...
            check: (ctx) => {
                ...
                if (elementCount > THRESHOLDS.DOM_ELEMENTS_POOR) {
                    return this.createFinding('large-dom', 'Very Large DOM', 'serious', `Page has ${elementCount} DOM elements (target: <${THRESHOLDS.MAX_DOM_ELEMENTS})`, 'Reduce DOM size by simplifying markup, using virtualization for long lists, and removing unnecessary wrappers', elementCount, THRESHOLDS.MAX_DOM_ELEMENTS);
                }
                else if (elementCount > THRESHOLDS.MAX_DOM_ELEMENTS) {
                    return this.createFinding('large-dom', 'Large DOM', 'moderate', `Page has ${elementCount} DOM elements (target: <${THRESHOLDS.MAX_DOM_ELEMENTS})`, 'Consider simplifying page structure to reduce DOM size', elementCount, THRESHOLDS.MAX_DOM_ELEMENTS);
                }
                return null;
            },
        },
            ...
            check: (ctx) => {
                ...
                ctx.$('link[href*="fonts.googleapis.com"]').each((_, el) => {
                    ...
                    if (!href.includes('display=swap')) {
                        findings.push(this.createFinding('missing-font-display', 'Google Fonts Missing display=swap', 'minor', 'Google Fonts loaded without display=swap parameter', 'Add &display=swap to your Google Fonts URL to prevent invisible text', undefined, undefined, `link[href*="fonts.googleapis"]`));
                    }
                });
                if (missingSwap > 0) {
                    findings.push(this.createFinding('missing-font-display', 'Missing font-display: swap', 'minor', `${missingSwap} @font-face declaration(s) missing font-display: swap`, 'Add font-display: swap to @font-face rules to show fallback text while fonts load', missingSwap));
                }
                return findings.length > 0 ? findings : null;
            },
        },
            ...
            check: (ctx) => {
                ...
                if (ctx.httpProtocol) {
                    ...
                    if (ctx.resources.length > 20) {
                        return this.createFinding('not-http2', 'Consider HTTP/2', 'info', `Page served over ${ctx.httpProtocol} and loads ${ctx.resources.length} resources -- HTTP/2 multiplexing would help`, 'Enable HTTP/2 on your server for better performance with multiple resources');
                    }
                    return null;
                }
                ...
                if (ctx.resources.length > 20) {
                    return this.createFinding('not-http2', 'Consider HTTP/2', 'info', `Page loads ${ctx.resources.length} resources over HTTP -- HTTP/2 multiplexing would help`, 'Enable HTTP/2 on your server for better performance with multiple resources');
                }
                return null;
            },
        },
        // Cache header analysis (#37)
        {
            ...
            check: (ctx) => {
                ...
                if (!cacheControl && !expires) {
                    return this.createFinding('missing-cache-headers', 'Missing Cache Headers', 'moderate', 'Response has no Cache-Control or Expires header', 'Add Cache-Control headers to enable browser caching and reduce server load');
                }
                if (cacheControl.includes('no-store') || cacheControl.includes('no-cache')) {
                    if (!cacheControl.includes('private')) {
                        return this.createFinding('cache-disabled', 'Caching Disabled', 'info', `Cache-Control: ${cacheControl}`, 'Consider enabling caching for static content to improve repeat visit performance');
                    }
                }
                if (!etag && !lastModified && !cacheControl.includes('immutable')) {
                    ...
                    return this.createFinding('missing-validation-headers', 'Missing Cache Validation', 'minor', 'Response lacks ETag and Last-Modified headers for cache validation', 'Add ETag or Last-Modified headers to enable conditional requests (304 responses)');
                }
                return null;
            },
        },
    ...
    async measureCoreWebVitals(page) {
        ...
        if (metrics.lcp !== null) {
            if (metrics.lcp > 4000) {
                findings.push(this.createFinding('lcp-poor', 'Poor LCP (Largest Contentful Paint)', 'serious', `LCP: ${Math.round(metrics.lcp)}ms (target: <2500ms)`, 'Optimize the largest content element (hero image, heading). Consider preloading, optimizing images, or reducing server response time.', metrics.lcp, 2500));
            }
            else if (metrics.lcp > 2500) {
                findings.push(this.createFinding('lcp-needs-improvement', 'LCP Needs Improvement', 'moderate', `LCP: ${Math.round(metrics.lcp)}ms (target: <2500ms)`, 'Optimize images, preload key resources, and reduce server response time to improve LCP.', metrics.lcp, 2500));
            }
        }
        // CLS thresholds: good < 0.1, poor > 0.25
        if (metrics.cls !== null) {
            if (metrics.cls > 0.25) {
                findings.push(this.createFinding('cls-poor', 'Poor CLS (Cumulative Layout Shift)', 'serious', `CLS: ${metrics.cls.toFixed(3)} (target: <0.1)`, 'Set explicit width/height on images and videos, avoid inserting content above existing content, and use CSS contain.', metrics.cls, 0.1));
            }
            else if (metrics.cls > 0.1) {
                findings.push(this.createFinding('cls-needs-improvement', 'CLS Needs Improvement', 'moderate', `CLS: ${metrics.cls.toFixed(3)} (target: <0.1)`, 'Add width/height to images, avoid dynamic content insertion, and reserve space for async content.', metrics.cls, 0.1));
            }
        }
        return findings;
    }
     ...
     */
    createFinding(ruleId, ruleName, severity, message, recommendation, metricValue, threshold, selector) {
    ...
    mobileRules = [
        // Missing viewport meta tag
        {
            ...
            check: (ctx) => {
                ...
                if (viewportMeta.length === 0) {
                    return this.createFinding('perf-mobile-viewport', 'Missing Viewport Meta Tag', 'critical', 'No <meta name="viewport"> found. Mobile browsers will render the page at desktop width and scale down.', 'Add <meta name="viewport" content="width=device-width, initial-scale=1"> to the <head>');
                }
                const content = viewportMeta.attr('content') || '';
                if (!content.includes('width=device-width')) {
                    return this.createFinding('perf-mobile-viewport', 'Viewport Meta Missing device-width', 'serious', `Viewport meta content is "${content}" but does not include width=device-width.`, 'Set viewport content to "width=device-width, initial-scale=1"');
                }
                return null;
            },
        },
        // Small font sizes on mobile
        {
            ...
            check: (ctx) => {
                ...
                if (fontMatch && parseInt(fontMatch[1]) < 16) {
                    return this.createFinding('perf-mobile-font-size', 'Small Base Font Size', 'moderate', `Body font-size is ${fontMatch[1]}px. Font sizes below 16px cause iOS Safari to zoom in on form inputs, hurting usability.`, 'Set body font-size to at least 16px for mobile, or use a responsive font-size with clamp()');
                }
                ...
                if (cssMatch && parseInt(cssMatch[1]) < 16) {
                    return this.createFinding('perf-mobile-font-size', 'Small Base Font Size', 'moderate', `Body CSS font-size is ${cssMatch[1]}px. Font sizes below 16px cause iOS Safari to zoom in on form inputs.`, 'Set body font-size to at least 16px for mobile');
                }
                return null;
            },
        },
            ...
            check: (ctx) => {
                ...
                ctx.$('table').each((_, el) => {
                    ...
                    if ((width && parseInt(width) > 500) || style.includes('width:') && !style.includes('max-width')) {
                        findings.push(this.createFinding('perf-mobile-overflow', 'Fixed-Width Table May Overflow', 'moderate', 'A table has a fixed width that may exceed the mobile viewport, causing horizontal scrolling.', 'Wrap tables in a scrollable container or use responsive table patterns'));
                    }
                });
                // Check for elements with inline fixed widths > 400px
                ctx.$('[style*="width"]').each((_, el) => {
                    const style = ctx.$(el).attr('style') || '';
                    const match = style.match(/width:\s*(\d+)px/);
                    if (match && parseInt(match[1]) > 500 && !style.includes('max-width')) {
                        const tag = el.tagName || 'element';
                        findings.push(this.createFinding('perf-mobile-overflow', 'Fixed-Width Element May Overflow', 'moderate', `A <${tag}> has inline width: ${match[1]}px which may exceed mobile viewport width.`, 'Use max-width instead of fixed width, or use responsive CSS'));
                    }
                });
                return findings.length > 0 ? findings : null;
            },
            ...
            check: (ctx) => {
                ...
                for (const img of imageResources) {
                    ...
                    if (imgEl.length > 0 && !imgEl.attr('srcset') && !imgEl.attr('sizes')) {
                        findings.push(this.createFinding('perf-mobile-images', 'Large Image Without Mobile Variant', 'moderate', `Image ${img.url.split('/').pop()} is ${Math.round(img.size / 1024)}KB and lacks srcset — mobile devices download the full desktop image.`, 'Add srcset and sizes attributes to serve smaller images on mobile', img.size, 200 * 1024));
                    }
                }
                return findings.length > 0 ? findings : null;
            },
        ...
        // 300ms tap delay
        {
            ...
            check: (ctx) => {
                ...
                return this.createFinding('perf-mobile-tap-delay', 'Potential 300ms Tap Delay', 'minor', 'Without width=device-width in the viewport meta tag, mobile browsers add a 300ms delay to touch events.', 'Add <meta name="viewport" content="width=device-width, initial-scale=1"> to eliminate the tap delay');
            },
        },
    ];
    /**
     ...
     */
    async analyzeMobile(crawlResult, page) {
        const $ = cheerio.load(crawlResult.html);
