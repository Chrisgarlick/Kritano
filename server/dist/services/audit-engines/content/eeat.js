...
exports.AUTHORITATIVE_DOMAINS = exports.CITATION_PHRASES = exports.MILLION_BILLION_REGEX = exports.LARGE_NUMBER_REGEX = exports.DATA_POINT_REGEX = void 0;
exports.analyzeEeat = analyzeEeat;
...
// =============================================
exports.DATA_POINT_REGEX = /\b\d+(\.\d+)?\s*(%|kg|lbs?|oz|g|mg|ml|L|°[FC]|mph|km\/h|GB|MB|TB|px|em|rem|ms|Hz|kHz|dB)\b/gi;
exports.LARGE_NUMBER_REGEX = /\b\d{1,3}(,\d{3})+\b/g;
exports.MILLION_BILLION_REGEX = /\b\d+(\.\d+)?\s*(million|billion|trillion)\b/gi;
exports.CITATION_PHRASES = [
    /\baccording to\b/i,
...
exports.AUTHORITATIVE_DOMAINS = ['.gov', '.edu', '.org'];
/**
 ...
 */
function analyzeEeat(html, text, sentences, words) {
    ...
    const experiencePhraseExamples = [];
    ...
    const technicalTermExamples = new Set();
    for (const word of words) {
        const clean = word.toLowerCase().replace(/[^a-z]/g, '');
        if (clean.length >= 4 && countSyllables(clean) >= 3 && !COMMON_WORDS.has(clean)) {
            technicalTermCount++;
            if (technicalTermExamples.size < 8)
                technicalTermExamples.add(clean);
        }
    }
    ...
    const dataPointMatches = [
        ...(text.match(exports.DATA_POINT_REGEX) || []),
        ...(text.match(exports.LARGE_NUMBER_REGEX) || []),
        ...(text.match(exports.MILLION_BILLION_REGEX) || []),
    ];
    ...
    for (const pattern of exports.CITATION_PHRASES) {
        const matches = text.match(pattern);
        if (matches) {
            citationPhraseCount += matches.length;
            for (const m of matches) {
                if (citationExamples.length < 3)
                    citationExamples.push(m);
            }
        }
    }
    ...
    const expertiseScore = Math.min(100, technicalTermDensity * 40 + dataPointCount * 5 + citationPhraseCount * 10);
    // =============================================
    ...
    const authLinkExamples = [];
    $('a[href]').each((_, el) => {
        const href = $(el).attr('href') || '';
        for (const domain of exports.AUTHORITATIVE_DOMAINS) {
            if (href.includes(domain)) {
                authoritativeLinkCount++;
                if (authLinkExamples.length < 3)
                    authLinkExamples.push(href.substring(0, 80));
                break;
            }
        }
    });
    ...
    // OVERALL SCORE
    // =============================================
    const overallScore = Math.round(experienceScore * 0.20 +
        expertiseScore * 0.25 +
