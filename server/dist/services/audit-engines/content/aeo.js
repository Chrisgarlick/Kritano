...
exports.analyzeAeo = analyzeAeo;
const cheerio = __importStar(require("cheerio"));
const eeat_js_1 = require("./eeat.js");
// =============================================
 ...
 */
function analyzeAeo(html, text, sentences, words) {
    ...
    const questionHeadingTexts = new Set();
    $('h2, h3').each((_, el) => {
    ...
    $('p').each((_, el) => {
        ...
        if (pWords.length < 30 || pWords.length > 70)
            return;
    ...
    $('p').each((_, el) => {
        const pText = $(el).text().trim();
        const pWords = pText.split(/\s+/).filter(w => w.length > 0);
        if (pWords.length < 15 || pWords.length > 40)
            return;
        // Must contain a factual claim (data point or named entity)
        const hasDataPoint = eeat_js_1.DATA_POINT_REGEX.test(pText) ||
            eeat_js_1.LARGE_NUMBER_REGEX.test(pText) ||
            eeat_js_1.MILLION_BILLION_REGEX.test(pText);
        // Reset lastIndex for global regexes
        eeat_js_1.DATA_POINT_REGEX.lastIndex = 0;
        eeat_js_1.LARGE_NUMBER_REGEX.lastIndex = 0;
        eeat_js_1.MILLION_BILLION_REGEX.lastIndex = 0;
    ...
    const nuggetScore = Math.min(100, Math.min(25, definitionBlockCount * 12.5) +
    ...
    const dataPointMatches = [
        ...(text.match(eeat_js_1.DATA_POINT_REGEX) || []),
        ...(text.match(eeat_js_1.LARGE_NUMBER_REGEX) || []),
        ...(text.match(eeat_js_1.MILLION_BILLION_REGEX) || []),
    ];
    // Reset lastIndex for global regexes
    eeat_js_1.DATA_POINT_REGEX.lastIndex = 0;
    eeat_js_1.LARGE_NUMBER_REGEX.lastIndex = 0;
    ...
    for (const sentence of sentences) {
        const hasData = eeat_js_1.DATA_POINT_REGEX.test(sentence) ||
            eeat_js_1.LARGE_NUMBER_REGEX.test(sentence) ||
            eeat_js_1.MILLION_BILLION_REGEX.test(sentence);
        eeat_js_1.DATA_POINT_REGEX.lastIndex = 0;
        eeat_js_1.LARGE_NUMBER_REGEX.lastIndex = 0;
        eeat_js_1.MILLION_BILLION_REGEX.lastIndex = 0;
        let hasCitation = false;
        for (const pattern of eeat_js_1.CITATION_PHRASES) {
            if (pattern.test(sentence)) {
                hasCitation = true;
                break;
            }
    ...
    const factualDensityScore = Math.min(100, hardNumberScore + namedEntityScore + verifiableClaimScore + specificDateScore);
    // =============================================
    ...
    $('script[type="application/ld+json"]').each((_, el) => {
        try {
            ...
            for (const item of items) {
                if (item['@type'] === 'HowTo')
                    hasHowToSchema = true;
                if (item['@type'] === 'ClaimReview')
                    hasClaimReviewSchema = true;
            }
        }
        catch {
            // ignore
    ...
    $('a[href]').each((_, el) => {
        ...
        for (const domain of eeat_js_1.AUTHORITATIVE_DOMAINS) {
    ...
    const sourceAuthorityScore = Math.min(100, (hasAuthorSameAs ? 25 : 0) +
        (hasFaqSchema ? 20 : 0) +
        (hasHowToSchema ? 15 : 0) +
    ...
    // OVERALL SCORE
    // =============================================
    const overallScore = Math.round(nuggetScore * 0.40 +
