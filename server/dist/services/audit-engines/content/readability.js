...
exports.getFleschKincaidGrade = getFleschKincaidGrade;
exports.getFleschReadingEase = getFleschReadingEase;
exports.getGunningFog = getGunningFog;
exports.getARI = getARI;
exports.getSentenceVariety = getSentenceVariety;
 ...
 */
function getGunningFog(words, sentenceCount) {
 ...
 */
function getARI(text, wordCount, sentenceCount) {
    if (wordCount === 0 || sentenceCount === 0)
        return 0;
 ...
 */
function getSentenceVariety(sentences) {
 ...
 */
function scoreGradeLevel(grade) {
    // Perfect score at grade 8
    const diff = Math.abs(grade - 8);
    if (diff <= 1)
        return 100; // 7-9: Perfect
    if (diff <= 2)
        return 85; // 6-10: Good
    if (diff <= 3)
        return 70; // 5-11: Acceptable
    if (diff <= 5)
        return 50; // 3-13: Difficult
    return 30; // Very difficult or very easy
}
/**
 * Main readability analysis function
 */
function analyzeReadability(text, sentences, words) {
    const findings = [];
    ...
    let score = Math.round(gradeScore * 0.35 +
        easeScore * 0.35 +
        fogScore * 0.15 +
        varietyScore * 0.15);
    // Generate findings
