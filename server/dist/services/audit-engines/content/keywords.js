...
exports.countKeywordOccurrences = countKeywordOccurrences;
exports.calculateDensity = calculateDensity;
exports.isInTitle = isInTitle;
exports.isInH1 = isInH1;
exports.isInFirstParagraph = isInFirstParagraph;
exports.isInLastParagraph = isInLastParagraph;
exports.isInMetaDescription = isInMetaDescription;
exports.isInUrl = isInUrl;
exports.isInAltText = isInAltText;
exports.findUsedVariations = findUsedVariations;
exports.detectKeywordStuffing = detectKeywordStuffing;
exports.scoreKeywordPlacement = scoreKeywordPlacement;
exports.scoreDensity = scoreDensity;
 ...
 */
function isInTitle(title, keyword) {
    if (!title)
        return false;
 ...
 */
function isInFirstParagraph(html, keyword) {
 ...
 */
function isInLastParagraph(html, keyword) {
    const $ = cheerio.load(html);
    const mainContent = $('main, article, [role="main"], .content, .post-content').first();
    const container = mainContent.length > 0 ? mainContent : $('body');
 ...
 */
function isInMetaDescription(metaDescription, keyword) {
    if (!metaDescription)
        return false;
 ...
 */
function isInAltText(html, keyword) {
    const $ = cheerio.load(html);
    const keywordLower = keyword.toLowerCase().trim();
    let found = false;
    $('img[alt]').each((_, el) => {
 ...
 */
function findUsedVariations(text, variations) {
    const used = [];
    const lower = text.toLowerCase();
 ...
 */
function detectKeywordStuffing(density, occurrences, wordCount) {
    ...
    if (wordCount < 500 && occurrences > 10)
        return true;
    return false;
}
/**
 ...
 */
function scoreKeywordPlacement(metrics) {
    let score = 0;
    ...
    if (metrics.inMetaDescription)
        score += 15;
    if (metrics.inUrl)
        score += 10;
    // Additional placements (lower weight)
    if (metrics.inAltText)
        score += 5;
    if (metrics.inLastParagraph)
        score += 5;
 ...
 */
function analyzeKeywords(text, pageData, targetKeyword) {
    const findings = [];
