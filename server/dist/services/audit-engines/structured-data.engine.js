...
exports.StructuredDataEngine = void 0;
exports.createStructuredDataEngine = createStructuredDataEngine;
const cheerio = __importStar(require("cheerio"));
const structured_data_types_js_1 = require("../../types/structured-data.types.js");
class StructuredDataEngine {
    rules = [
        // Critical: No structured data at all
        {
            ...
            check: (ctx) => {
                if (!ctx.analysis.jsonLd.found && !ctx.analysis.microdata.found) {
                    return this.createFinding('no-structured-data', 'Missing Structured Data', 'critical', 'No structured data (JSON-LD or Microdata) found on this page. You are missing out on Rich Snippets in search results.', 'Add JSON-LD structured data to help search engines understand your content. Start with Organization schema for your homepage, and Article/Product schemas for content pages.', undefined, undefined, undefined);
                }
                return null;
            },
        },
        // Critical: Invalid JSON-LD syntax
        {
            ...
            check: (ctx) => {
                ...
                if (invalidItems.length === 0)
                    return null;
                return invalidItems.map(item => this.createFinding('invalid-json-ld-syntax', 'Invalid JSON-LD Syntax', 'critical', `JSON-LD contains syntax errors: ${item.parseError}`, 'Fix the JSON syntax errors in your structured data. Use a JSON validator to identify and correct the issues.', 'script[type="application/ld+json"]', item.raw.substring(0, 200) + (item.raw.length > 200 ? '...' : ''), undefined));
            },
        },
                    ...
                    if (item.isValid && item.parsed) {
                        ...
                        for (const obj of items) {
                            if (obj && typeof obj === 'object' && !obj['@type']) {
                                findings.push(this.createFinding('missing-schema-type', 'Missing Schema Type', 'serious', 'JSON-LD object is missing the required @type property', 'Add a @type property to specify the schema type (e.g., "Organization", "Article", "Product")', 'script[type="application/ld+json"]', JSON.stringify(obj).substring(0, 150) + '...', undefined));
                            }
                        }
                    }
                }
        ...
        // Serious: Homepage missing Organization/WebSite schema
        {
            ...
            check: (ctx) => {
                if (!ctx.isHomepage)
                    return null;
                const hasOrgOrWebsite = ctx.analysis.detectedTypes.some(t => t === 'Organization' || t === 'WebSite');
                if (!hasOrgOrWebsite && ctx.analysis.jsonLd.found) {
                    return this.createFinding('homepage-missing-organization', 'Homepage Missing Organization Schema', 'serious', 'Your homepage has structured data but is missing Organization or WebSite schema', 'Add Organization schema with your company name, logo, contact information, and social profiles to establish your brand in search results.', undefined, undefined, 'Organization');
                }
                return null;
            },
        },
            ...
            check: (ctx) => {
                ...
                for (const item of ctx.analysis.jsonLd.items) {
                    ...
                    for (const obj of items) {
                        ...
                        const requirements = structured_data_types_js_1.SCHEMA_REQUIREMENTS[schemaType];
                        if (!requirements)
                            continue;
                        const missingRecommended = requirements.recommended.filter(field => !obj[field]);
                        if (missingRecommended.length > 0) {
                            findings.push(this.createFinding('missing-recommended-fields', 'Missing Recommended Fields', 'moderate', `${schemaType} schema is missing recommended fields: ${missingRecommended.slice(0, 3).join(', ')}${missingRecommended.length > 3 ? ` and ${missingRecommended.length - 3} more` : ''}`, `Add the missing fields to improve your chances of getting rich snippets: ${missingRecommended.join(', ')}`, undefined, undefined, schemaType));
                        }
                    }
                }
            ...
            check: (ctx) => {
                ...
                for (const item of ctx.analysis.jsonLd.items) {
                    ...
                    for (const obj of items) {
                        ...
                        if (!requirements)
                            continue;
                        const missingRequired = requirements.required.filter(field => field !== '@type' && !obj[field]);
                        if (missingRequired.length > 0) {
                            findings.push(this.createFinding('missing-required-fields', 'Missing Required Fields', 'serious', `${schemaType} schema is missing required fields: ${missingRequired.join(', ')}`, `Add the required fields to ensure your structured data is valid: ${missingRequired.join(', ')}`, undefined, undefined, schemaType));
                        }
                    }
                }
        ...
        // Moderate: Missing Open Graph data
        {
            ...
            check: (ctx) => {
                ...
                const missingTags = [];
                if (!ctx.analysis.openGraph.data.title)
                    missingTags.push('og:title');
                if (!ctx.analysis.openGraph.data.description)
                    missingTags.push('og:description');
                if (!ctx.analysis.openGraph.data.image)
                    missingTags.push('og:image');
                if (!ctx.analysis.openGraph.data.url)
                    missingTags.push('og:url');
                if (missingTags.length === 0)
                    return null;
                return this.createFinding('missing-open-graph', 'Missing Open Graph Data', 'moderate', `Missing Open Graph tags for social sharing: ${missingTags.join(', ')}`, 'Add Open Graph meta tags to control how your content appears when shared on social media platforms like Facebook and LinkedIn.', 'head', undefined, undefined);
            },
        },
        // Minor: Missing Twitter Card data
        {
            ...
            check: (ctx) => {
                ...
                return this.createFinding('missing-twitter-card', 'Missing Twitter Card Data', 'minor', 'Missing Twitter Card meta tags for Twitter/X sharing', 'Add Twitter Card meta tags (twitter:card, twitter:title, twitter:description, twitter:image) for better Twitter sharing previews.', 'head', undefined, undefined);
            },
        },
        // Info: Schema type suggestions based on page content
        {
            ...
            helpUrl: 'https://schema.org/',
            check: (ctx) => {
                if (!ctx.analysis.detectedPageType)
                    return null;
                if (ctx.analysis.missingExpectedTypes.length === 0)
                    return null;
                const missing = ctx.analysis.missingExpectedTypes;
                return this.createFinding('schema-suggestion', 'Schema Type Suggestion', 'info', `Based on your page content (${ctx.analysis.detectedPageType}), consider adding: ${missing.join(', ')} schema`, `Adding ${missing.join(' and ')} schema can help your page qualify for rich snippets specific to this content type.`, undefined, undefined, missing[0]);
            },
        },
        // Info: Product page without offers
        {
            ...
            check: (ctx) => {
                ...
                if (hasProduct && !hasOffer) {
                    ...
                    return this.createFinding('product-missing-offer', 'Product Missing Offer Schema', 'moderate', 'Product schema found but missing Offer information (price, availability)', 'Add Offer schema with price, priceCurrency, and availability to display pricing in search results.', undefined, undefined, 'Product');
                }
                return null;
            },
        },
                ...
                for (const item of ctx.analysis.jsonLd.items) {
                    ...
                    for (const obj of items) {
                        if (obj && articleTypes.includes(obj['@type']) && !obj.author) {
                            return this.createFinding('article-missing-author', 'Article Missing Author', 'moderate', 'Article schema is missing author information, which is important for E-E-A-T signals', 'Add author property with name and optionally url to establish content credibility.', undefined, undefined, obj['@type']);
                        }
                    }
                }
                return null;
    ...
    async analyze(crawlResult) {
        ...
        const ctx = {
            ...
            $: $, // Type compatibility with StructuredDataContext
            analysis,
            isHomepage,
        };
        // Run all rules
     ...
     */
    getDetectedTypes(jsonLdItems, microdataItems) {
        const types = new Set();
     ...
     * Detect page type and expected schemas
     */
    detectPageTypeAndExpectations($, detectedTypes, url) {
     ...
     */
    calculateScore(jsonLdItems, openGraph, detectedTypes, missingTypes) {
        let score = 0;
        // Presence (40 points)
     ...
     */
    calculateOpenGraphCompleteness(og) {
        const fields = ['title', 'description', 'image', 'url', 'type', 'siteName'];
        const present = fields.filter(f => !!og[f]).length;
        return Math.round((present / fields.length) * 100);
    }
    /**
     * Calculate Twitter Card completeness
     */
    calculateTwitterCardCompleteness(tc) {
        const fields = ['card', 'title', 'description', 'image'];
        const present = fields.filter(f => !!tc[f]).length;
        return Math.round((present / fields.length) * 100);
    }
    /**
     * Get validation errors from JSON-LD
     */
    getValidationErrors(jsonLdItems) {
        const errors = [];
        for (const item of jsonLdItems) {
            if (!item.isValid) {
                errors.push({
     ...
     */
    createFinding(ruleId, ruleName, severity, message, recommendation, selector, snippet, schemaType) {
     ...
     */
    getAnalysis(crawlResult) {
        const $ = cheerio.load(crawlResult.html);
        return this.analyzeStructuredData($, crawlResult.url);
    }
}
exports.StructuredDataEngine = StructuredDataEngine;
/**
 * Factory function to create the engine
 */
function createStructuredDataEngine() {
    return new StructuredDataEngine();
}
//# sourceMappingURL=structured-data.engine.js.map