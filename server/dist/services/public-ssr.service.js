...
exports.renderServicesPage = renderServicesPage;
exports.renderServiceDetailPage = renderServiceDetailPage;
exports.renderPricingPage = renderPricingPage;
exports.renderContactPage = renderContactPage;
exports.renderFaqPage = renderFaqPage;
exports.renderAuthorPage = renderAuthorPage;
...
function renderHomepage() {
    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-20 py-12">
      <div class="max-w-3xl mx-auto">
        ...
        ${(0, ssr_shared_service_js_1.renderAuthorBio)()}
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: ssr_shared_service_js_1.BASE_URL,
...
function renderAboutPage() {
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/about`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-about.png`,
        ogType: 'website',
        extraHead: structuredData,
        body,
        activePath: '/about',
...
const SERVICES_DATA = {
    seo: {
        ...
        seo: {
            ...
            description: 'Analyse your website against 100+ SEO ranking factors including metadata, structured data, Core Web Vitals, and mobile-friendliness.',
        },
    ...
    'content-quality': {
        ...
        seo: {
            ...
            description: 'Score your pages on readability, structure, engagement, and E-E-A-T. Kritano flags thin content, weak headings, and missing author signals that hurt rankings.',
        },
...
function getAccentColor(colorScheme) {
    if (colorScheme.text.includes('violet'))
        return ACCENT_COLORS.violet;
    if (colorScheme.text.includes('emerald'))
        return ACCENT_COLORS.emerald;
    if (colorScheme.text.includes('red'))
        return ACCENT_COLORS.red;
    if (colorScheme.text.includes('amber'))
...
function renderServicesPage() {
    ...
    const serviceCards = SERVICES_LISTING.map((service) => {
        ...
        const featuresHtml = service.features.map(f => `<div class="flex items-center gap-2">${checkCircleSvg}<span class="text-xs text-slate-600">${(0, ssr_shared_service_js_1.escapeHtml)(f)}</span></div>`).join('\n                    ');
        return `
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <h2 class="text-lg font-semibold text-slate-900">
                        <a href="/services/${service.id}" class="hover:text-indigo-600 transition-colors">${(0, ssr_shared_service_js_1.escapeHtml)(data.title)}</a>
                      </h2>
                      <p class="text-sm text-indigo-600 font-medium">${(0, ssr_shared_service_js_1.escapeHtml)(data.subtitle)}</p>
                    </div>
                  </div>
                  <div class="text-right flex-shrink-0">
                    <p class="font-display text-2xl text-slate-900">${(0, ssr_shared_service_js_1.escapeHtml)(service.stat)}</p>
                    <p class="text-[10px] text-slate-600 uppercase tracking-wider font-medium">${(0, ssr_shared_service_js_1.escapeHtml)(service.statLabel)}</p>
                  </div>
                </div>
                <p class="text-sm text-slate-600 leading-relaxed mb-6">${(0, ssr_shared_service_js_1.escapeHtml)(service.description)}</p>
    ...
    const faqsHtml = SERVICE_FAQS.map(faq => `
          <details class="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <summary class="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-slate-50 transition-colors">
              <span class="font-semibold text-slate-900 text-sm pr-4">${(0, ssr_shared_service_js_1.escapeHtml)(faq.q)}</span>
            ...
            <div class="px-5 pb-4">
              <p class="text-sm text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(faq.a)}</p>
            </div>
          </details>`).join('\n');
    const toolCards = [
    ...
    const toolCardsHtml = toolCards.map(item => `
              <div class="bg-white border border-slate-200 rounded-xl p-5 text-center hover:shadow-sm transition-shadow">
                ...
                <h3 class="font-semibold text-slate-900 mb-1.5 text-sm">${(0, ssr_shared_service_js_1.escapeHtml)(item.title)}</h3>
                <p class="text-slate-600 text-xs leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(item.description)}</p>
              </div>`).join('\n');
    const structuredData = [
        jsonLd({
            '@context': 'https://schema.org',
...
${faqsHtml}
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/services`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-services.png`,
        ogType: 'website',
        extraHead: structuredData,
        body,
        activePath: '/services',
...
function renderServiceDetailPage(slug) {
    ...
    const statCardsHtml = service.businessImpact.stats.map(stat => `
                <div class="bg-white border border-slate-200 rounded-xl p-5 text-center">
                  <p class="font-display text-3xl mb-1" style="color: ${accentColor}">${(0, ssr_shared_service_js_1.escapeHtml)(stat.value)}</p>
                  <p class="text-xs text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(stat.label)}</p>
                </div>`).join('\n');
    // Definition block
    const definitionTitle = service.title.toLowerCase().replace(/\(.*\)/, '').trim();
    // Feature groups
    const featureGroupsHtml = service.featureGroups.map(group => {
        const itemsHtml = group.items.map(item => `<li class="flex items-start gap-2.5"><span style="color: ${accentColor}">${checkCircle}</span><span class="text-slate-600 text-sm leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(item)}</span></li>`).join('\n                    ');
        return `
              <div class="bg-white border border-slate-200 rounded-xl p-6">
                <h3 class="font-semibold text-slate-900 mb-4 text-sm uppercase tracking-wider">${(0, ssr_shared_service_js_1.escapeHtml)(group.title)}</h3>
    ...
    const methodologyHtml = service.methodology.map((step, i) => {
        ...
        return `
              <div class="relative flex flex-col items-center text-center px-4 py-6">
                ...
                <h3 class="font-semibold text-slate-900 mb-2 text-sm">${(0, ssr_shared_service_js_1.escapeHtml)(step.title)}</h3>
                <p class="text-slate-600 text-xs leading-relaxed max-w-[200px]">${(0, ssr_shared_service_js_1.escapeHtml)(step.description)}</p>
              </div>`;
    }).join('\n');
    // Common issues
              ...
              <div class="bg-white border border-slate-200 rounded-lg p-5 flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    <h3 class="font-semibold text-slate-900 text-sm">${(0, ssr_shared_service_js_1.escapeHtml)(issue.title)}</h3>
                    <span class="px-1.5 py-0.5 rounded text-[10px] font-medium capitalize ${sev.bg} ${sev.text}">${(0, ssr_shared_service_js_1.escapeHtml)(issue.severity)}</span>
                  </div>
                  <p class="text-slate-600 text-xs leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(issue.description)}</p>
                </div>
              </div>`;
    }).join('\n');
    // Key takeaways
    const takeawaysHtml = service.keyTakeaways.map(t => `<li class="flex items-start gap-2.5"><svg class="w-4 h-4 text-indigo-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span class="text-slate-700 text-sm leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(t)}</span></li>`).join('\n              ');
    // FAQs
          ...
          <details class="bg-white border border-slate-200 rounded-lg overflow-hidden">
            <summary class="w-full flex items-center justify-between gap-4 p-5 cursor-pointer">
              <div class="flex items-start gap-3">
                ...
                <span class="font-semibold text-slate-900 text-sm">${(0, ssr_shared_service_js_1.escapeHtml)(faq.question)}</span>
              </div>
            ...
            <div class="px-5 pb-5 pt-0 pl-[3.25rem]">
              <p class="text-sm text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(faq.answer)}</p>
            </div>
          </details>`).join('\n');
    // Related services
    const relatedHtml = service.relatedSlugs.map(relSlug => {
        ...
        return `
              <a href="/services/${relSlug}" class="bg-white border border-slate-200 rounded-xl p-6 hover:shadow-md transition-all block">
                ...
                <h3 class="font-semibold text-slate-900 mb-1">${(0, ssr_shared_service_js_1.escapeHtml)(related.title)}</h3>
                <p class="text-slate-600 text-sm mb-3">${(0, ssr_shared_service_js_1.escapeHtml)(related.subtitle)}</p>
      ...
      <div class="max-w-7xl mx-auto px-6 lg:px-20 pt-20 lg:pt-28 pb-16">
        <nav aria-label="Breadcrumb" class="mb-8">
          <ol class="flex items-center gap-2 text-sm text-slate-600">
            ...
            <li class="text-slate-900 font-medium">${(0, ssr_shared_service_js_1.escapeHtml)(service.title)}</li>
          </ol>
        </nav>
        ...
        <div class="grid lg:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <h1 class="font-display text-5xl lg:text-6xl text-slate-900 leading-[1.05] mb-4">
              ${(0, ssr_shared_service_js_1.escapeHtml)(service.title)}
            </h1>
            <p class="text-xl font-medium mb-6" style="color: ${accentColor}">
              ${(0, ssr_shared_service_js_1.escapeHtml)(service.subtitle)}
            </p>
            <p class="text-lg text-slate-600 leading-relaxed mb-8">
              ${(0, ssr_shared_service_js_1.escapeHtml)(service.heroDescription)}
            </p>
      ...
      <div class="max-w-7xl mx-auto px-6 lg:px-20 py-10">
        <div class="max-w-3xl mx-auto">
          <p class="text-lg text-slate-700 leading-relaxed">
            <strong class="text-slate-900">What is ${(0, ssr_shared_service_js_1.escapeHtml)(definitionTitle)}?</strong> ${(0, ssr_shared_service_js_1.escapeHtml)(service.definition)}
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-20 py-20">
      <div class="max-w-3xl mx-auto text-center">
        <h2 class="font-display text-4xl text-slate-900 leading-tight mb-4">
          ${(0, ssr_shared_service_js_1.escapeHtml)(service.businessImpact.headline)}
        </h2>
        <p class="text-lg text-slate-600 leading-relaxed">
          ${(0, ssr_shared_service_js_1.escapeHtml)(service.businessImpact.description)}
        </p>
      </div>
    </section>
...
${relatedHtml}
    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-20 py-20">
      <div class="bg-indigo-600 rounded-2xl p-10 md:p-14 relative overflow-hidden">
        ...
        <div class="relative text-center max-w-xl mx-auto">
          <h2 class="font-display text-3xl lg:text-4xl text-white leading-tight mb-5">
            ${(0, ssr_shared_service_js_1.escapeHtml)(service.cta.headline)}
          </h2>
          <p class="text-indigo-200 leading-relaxed mb-8">
            ${(0, ssr_shared_service_js_1.escapeHtml)(service.cta.description)}
          </p>
          <a href="/register?ea=email" class="inline-flex items-center gap-2 px-7 py-3.5 bg-white text-indigo-700 rounded-lg font-semibold hover:bg-indigo-50 transition-colors">
            Join Early Access ${arrowRight}
          </a>
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        description: service.seo.description,
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/services/${slug}`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}${service.ogImage}`,
        ogType: 'website',
        extraHead: structuredData,
        body,
        activePath: '/services',
...
function textCell(val, isProCol = false) {
    return `<td class="px-4 py-3 text-sm text-slate-700 text-center${isProCol ? ' bg-indigo-50' : ''}">${(0, ssr_shared_service_js_1.escapeHtml)(val)}</td>`;
}
function boolRow(label, vals) {
    return `<tr class="border-b border-slate-100">
    <td class="px-4 py-3 text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(label)}</td>
...
function textRow(label, vals) {
    return `<tr class="border-b border-slate-100">
    <td class="px-4 py-3 text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(label)}</td>
    <td class="px-4 py-3 text-sm text-slate-700 text-center">${(0, ssr_shared_service_js_1.escapeHtml)(vals[0])}</td>
    <td class="px-4 py-3 text-sm text-slate-700 text-center">${(0, ssr_shared_service_js_1.escapeHtml)(vals[1])}</td>
    <td class="px-4 py-3 text-sm text-slate-700 text-center bg-indigo-50">${(0, ssr_shared_service_js_1.escapeHtml)(vals[2])}</td>
    <td class="px-4 py-3 text-sm text-slate-700 text-center">${(0, ssr_shared_service_js_1.escapeHtml)(vals[3])}</td>
    <td class="px-4 py-3 text-sm text-slate-700 text-center">${(0, ssr_shared_service_js_1.escapeHtml)(vals[4])}</td>
  </tr>`;
}
function sectionHeader(title) {
    return `<tr class="bg-slate-50">
    <td colspan="6" class="px-4 py-3 text-sm font-semibold text-slate-900">${(0, ssr_shared_service_js_1.escapeHtml)(title)}</td>
  </tr>`;
}
function renderPricingPage() {
  ...
  <table class="w-full text-left border-collapse">
    ...
    const faqHtml = faqs.map(f => `<details class="border border-slate-200 rounded-lg">
        <summary class="px-5 py-4 cursor-pointer text-sm font-medium text-slate-900 hover:bg-slate-50">${(0, ssr_shared_service_js_1.escapeHtml)(f.q)}</summary>
        <div class="px-5 pb-4 text-sm text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(f.a)}</div>
      </details>`).join('\n      ');
    ...
    const structuredData = [
        jsonLd({
            ...
            offers: [
                { '@type': 'Offer', name: 'Free', price: '0', priceCurrency: 'USD', url: `${ssr_shared_service_js_1.BASE_URL}/pricing` },
                { '@type': 'Offer', name: 'Starter', price: '19', priceCurrency: 'USD', url: `${ssr_shared_service_js_1.BASE_URL}/pricing` },
                { '@type': 'Offer', name: 'Pro', price: '49', priceCurrency: 'USD', url: `${ssr_shared_service_js_1.BASE_URL}/pricing` },
                { '@type': 'Offer', name: 'Agency', price: '99', priceCurrency: 'USD', url: `${ssr_shared_service_js_1.BASE_URL}/pricing` },
                { '@type': 'Offer', name: 'Enterprise', price: '0', priceCurrency: 'USD', url: `${ssr_shared_service_js_1.BASE_URL}/pricing`, description: 'Custom pricing' },
            ],
        }),
        jsonLd({
            '@context': 'https://schema.org',
        ...
        jsonLd({
            ...
            '@type': 'BreadcrumbList',
            itemListElement: [
                { '@type': 'ListItem', position: 1, name: 'Home', item: ssr_shared_service_js_1.BASE_URL },
                { '@type': 'ListItem', position: 2, name: 'Pricing', item: `${ssr_shared_service_js_1.BASE_URL}/pricing` },
            ],
        }),
    ].join('\n  ');
    const body = `<main id="main-content" aria-label="Page content">
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/pricing`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-default.png`,
        ogType: 'website',
        extraHead: structuredData,
        body,
        activePath: '/pricing',
...
function renderContactPage() {
    const structuredData = [
        jsonLd({
            ...
            url: `${ssr_shared_service_js_1.BASE_URL}/contact`,
            mainEntity: {
                '@type': 'Organization',
                name: 'Kritano',
                email: 'info@kritano.com',
        ...
        jsonLd({
            ...
            itemListElement: [
                { '@type': 'ListItem', position: 1, name: 'Home', item: ssr_shared_service_js_1.BASE_URL },
                { '@type': 'ListItem', position: 2, name: 'Contact', item: `${ssr_shared_service_js_1.BASE_URL}/contact` },
            ],
        }),
    ].join('\n  ');
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/contact`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-default.png`,
        ogType: 'website',
        extraHead: structuredData,
        body,
...
function renderFaqPage() {
    ...
    const structuredData = [
        ...
        jsonLd({
            ...
            itemListElement: [
                { '@type': 'ListItem', position: 1, name: 'Home', item: ssr_shared_service_js_1.BASE_URL },
                { '@type': 'ListItem', position: 2, name: 'FAQ', item: `${ssr_shared_service_js_1.BASE_URL}/faq` },
            ],
        }),
    ].join('\n  ');
    const categorySections = categories.map(cat => {
        ...
        <h2 class="font-display text-2xl text-slate-900 mb-6">${(0, ssr_shared_service_js_1.escapeHtml)(cat.title)}</h2>
        <div class="space-y-3">
          ${faqItems}
        </div>
      </div>`;
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/faq`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-default.png`,
        ogType: 'website',
        extraHead: structuredData,
        body,
...
function renderAuthorPage() {
    ...
    const structuredData = [
        jsonLd({
            ...
            name: 'Chris Garlick',
            jobTitle: 'Founder',
            url: `${ssr_shared_service_js_1.BASE_URL}/author/chris-garlick`,
            worksFor: {
                '@type': 'Organization',
                name: 'Kritano',
                url: ssr_shared_service_js_1.BASE_URL,
        ...
        jsonLd({
            ...
            itemListElement: [
                { '@type': 'ListItem', position: 1, name: 'Home', item: ssr_shared_service_js_1.BASE_URL },
                { '@type': 'ListItem', position: 2, name: 'Chris Garlick', item: `${ssr_shared_service_js_1.BASE_URL}/author/chris-garlick` },
            ],
        }),
    ].join('\n  ');
    const expertiseTags = expertiseAreas.map(area => `<span class="inline-block px-3 py-1.5 bg-indigo-50 text-indigo-700 text-sm rounded-full border border-indigo-100">${(0, ssr_shared_service_js_1.escapeHtml)(area)}</span>`).join('\n            ');
    const body = `<main id="main-content" aria-label="Page content">
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        description: 'Chris Garlick is the founder of Kritano, a website intelligence platform. He writes about SEO, web accessibility, security, and performance.',
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/author/chris-garlick`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-default.png`,
        ogType: 'profile',
        extraHead: structuredData,
        body,
...
//# sourceMappingURL=public-ssr.service.js.map