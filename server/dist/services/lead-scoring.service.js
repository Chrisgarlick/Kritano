...
exports.getLeadProfile = getLeadProfile;
exports.getLeadTimeline = getLeadTimeline;
exports.getLeadMemberships = getLeadMemberships;
exports.getLeadStats = getLeadStats;
exports.batchRecalculate = batchRecalculate;
...
async function recalculateScore(userId) {
    ...
    try {
        ...
        FROM audit_jobs WHERE user_id = $1`, [userId]),
            client.query(`SELECT COUNT(*) as count FROM sites WHERE created_by = $1`, [userId]),
            client.query(`SELECT COUNT(*) as count
         FROM sites s
         WHERE s.created_by = $1 AND s.verified = true`, [userId]),
            client.query(`SELECT COUNT(DISTINCT ss.user_id) as count
         FROM site_shares ss
         JOIN sites s ON ss.site_id = s.id
         WHERE s.created_by = $1 AND ss.user_id != $1`, [userId]),
            client.query(`SELECT
       ...
       LIMIT 1`, [userId]);
        if (tierLimitResult.rows.length > 0) {
            ...
            if (limits.max_domains && siteCount >= limits.max_domains)
                score += 40;
            // Monthly audit limit check
            const monthlyAudits = await client.query(`SELECT COUNT(*) as count FROM audit_jobs
         WHERE user_id = $1 AND created_at >= date_trunc('month', NOW())`, [userId]);
            const monthCount = parseInt(monthlyAudits.rows[0].count) || 0;
            if (limits.max_audits_per_month && monthCount >= limits.max_audits_per_month)
                score += 35;
        }
        // Inactivity decay
        ...
        const tierResult = await client.query(`SELECT sub.tier::text FROM subscriptions sub
       JOIN sites s ON s.organization_id = sub.organization_id
       WHERE s.created_by = $1
       LIMIT 1`, [userId]);
        ...
        // Save
        await client.query(`UPDATE users SET lead_score = $1, lead_status = $2, lead_score_updated_at = NOW() WHERE id = $3`, [score, finalStatus, userId]);
 ...
 */
function deriveStatus(score, lastLoginAt) {
    if (lastLoginAt) {
 ...
 */
async function getLeadBoard(filters) {
    const { status, search, sort = 'lead_score', order = 'desc', page = 1, limit = 50, } = filters;
    ...
    const [dataResult, countResult] = await Promise.all([
      ...
      LIMIT $${paramIdx++} OFFSET $${paramIdx++}`, [...params, limit, offset]),
        index_js_1.pool.query(`SELECT COUNT(*) FROM users u ${where}`, params),
    ]);
    return {
 ...
 */
async function getLeadProfile(userId) {
 ...
 */
async function getLeadTimeline(userId) {
    const events = [];
    // Registration
    const userResult = await index_js_1.pool.query(`SELECT created_at, email_verified, last_login_at FROM users WHERE id = $1`, [userId]);
    ...
    const audits = await index_js_1.pool.query(`SELECT id, target_url, target_domain, status, created_at, completed_at,
    ...
    const domains = await index_js_1.pool.query(`SELECT s.domain, s.verified_at FROM sites s
     WHERE s.created_by = $1 AND s.verified = true AND s.verified_at IS NOT NULL
     ORDER BY s.verified_at ASC`, [userId]);
    ...
    // Team members added
    const members = await index_js_1.pool.query(`SELECT ss.created_at, u.email, s.domain
     FROM site_shares ss
     JOIN sites s ON ss.site_id = s.id
     JOIN users u ON ss.user_id = u.id
     WHERE s.created_by = $1 AND ss.user_id != $1
     ORDER BY ss.created_at ASC`, [userId]);
    ...
    // Triggers
    const triggers = await index_js_1.pool.query(`SELECT trigger_type, context, created_at FROM crm_triggers
 ...
 */
async function getLeadMemberships(userId) {
    const result = await index_js_1.pool.query(`SELECT
...
async function batchRecalculate() {
    const result = await index_js_1.pool.query(`SELECT id FROM users WHERE status != 'deleted' ORDER BY id`);
    let processed = 0;
    let errors = 0;
