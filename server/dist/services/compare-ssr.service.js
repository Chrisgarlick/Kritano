...
exports.renderCompareLanding = renderCompareLanding;
exports.renderCompareDetail = renderCompareDetail;
...
function renderFeatureCell(value) {
    if (value === true)
        return `<td class="px-4 py-3 text-center">${CHECK_SVG}<span class="sr-only">Yes</span></td>`;
    if (value === false)
        return `<td class="px-4 py-3 text-center">${X_SVG}<span class="sr-only">No</span></td>`;
    return `<td class="px-4 py-3 text-center text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(value)}</td>`;
}
function renderComparisonTable(entry) {
    const competitorLabel = entry.competitor;
    let html = '';
    for (const category of entry.categories) {
        html += `
    <div class="mb-8">
      <h3 class="text-lg font-semibold text-slate-900 mb-4">${(0, ssr_shared_service_js_1.escapeHtml)(category.name)}</h3>
      <div class="overflow-x-auto">
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-slate-50">
              ...
              <th scope="col" class="px-4 py-3 text-center text-sm font-semibold text-slate-700 w-1/4">${(0, ssr_shared_service_js_1.escapeHtml)(competitorLabel)}</th>
            </tr>
          </thead>
          <tbody>`;
        for (let i = 0; i < category.features.length; i++) {
            ...
            <tr class="${rowClass} border-t border-slate-100">
              <td class="px-4 py-3 text-sm text-slate-900">${(0, ssr_shared_service_js_1.escapeHtml)(feature.name)}</td>
  ...
  <section class="mb-12" aria-labelledby="top-alternatives">
    ...
    for (let i = 0; i < alternatives.length; i++) {
        ...
        const linkAttrs = isKritano
            ? `href="/pricing"`
            : `href="${(0, ssr_shared_service_js_1.escapeHtml)(alt.url)}" target="_blank" rel="noopener noreferrer nofollow"`;
        html += `
      <div class="border border-slate-200 rounded-lg p-6 bg-white shadow-sm${isKritano ? ' ring-2 ring-indigo-500/20 border-indigo-200' : ''}">
        <div class="flex items-start gap-3 mb-3">
          ...
          <h3 class="text-lg font-semibold text-slate-900">${(0, ssr_shared_service_js_1.escapeHtml)(alt.name)}${isKritano ? ' <span class="text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full ml-2">Recommended</span>' : ''}</h3>
        </div>
        <p class="text-sm text-slate-600 mb-3">${(0, ssr_shared_service_js_1.escapeHtml)(alt.description)}</p>
        <dl class="text-sm space-y-1 mb-4">
          <div class="flex gap-2"><dt class="font-medium text-slate-700">Pricing:</dt><dd class="text-slate-600">${(0, ssr_shared_service_js_1.escapeHtml)(alt.pricing)}</dd></div>
          <div class="flex gap-2"><dt class="font-medium text-slate-700">Best for:</dt><dd class="text-slate-600">${(0, ssr_shared_service_js_1.escapeHtml)(alt.bestFor)}</dd></div>
        </dl>
        <a ${linkAttrs} class="text-sm font-medium text-indigo-600 hover:text-indigo-700 transition-colors">Visit ${(0, ssr_shared_service_js_1.escapeHtml)(alt.name)} &rarr;</a>
      </div>`;
    }
    html += `
    </div>
...
function renderPricingSection(entry) {
    return `
  <section class="mb-12" aria-labelledby="pricing-comparison">
    ...
    <div class="grid gap-6 md:grid-cols-2 mb-6">
      <div class="border border-indigo-200 rounded-lg p-6 bg-indigo-50/30">
        ...
        <p class="text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(entry.pricing.kritano)}</p>
      </div>
      <div class="border border-slate-200 rounded-lg p-6 bg-slate-50">
        <h3 class="font-semibold text-slate-900 mb-2">${(0, ssr_shared_service_js_1.escapeHtml)(entry.competitor)}</h3>
        <p class="text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(entry.pricing.competitor)}</p>
      </div>
    </div>
    <div class="border border-slate-200 rounded-lg p-6 bg-white">
      <h3 class="font-semibold text-slate-900 mb-2">Verdict</h3>
      <p class="text-sm text-slate-700">${(0, ssr_shared_service_js_1.escapeHtml)(entry.pricing.verdict)}</p>
    </div>
  </section>`;
}
function renderBestForSection(entry) {
    return `
  <section class="mb-12" aria-labelledby="best-for">
    ...
    <div class="grid gap-6 md:grid-cols-2">
      <div class="border border-indigo-200 rounded-lg p-6 bg-indigo-50/30">
        ...
        <ul class="space-y-2">
          ${entry.bestFor.kritano.map(item => `<li class="flex items-start gap-2 text-sm text-slate-700">${CHECK_SVG}<span>${(0, ssr_shared_service_js_1.escapeHtml)(item)}</span></li>`).join('\n          ')}
        </ul>
      </div>
      <div class="border border-slate-200 rounded-lg p-6 bg-slate-50">
        <h3 class="font-semibold text-slate-900 mb-3">Choose ${(0, ssr_shared_service_js_1.escapeHtml)(entry.competitor)} if...</h3>
        <ul class="space-y-2">
          ${entry.bestFor.competitor.map(item => `<li class="flex items-start gap-2 text-sm text-slate-700"><svg class="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg><span>${(0, ssr_shared_service_js_1.escapeHtml)(item)}</span></li>`).join('\n          ')}
        </ul>
      </div>
    </div>
  </section>`;
}
function renderFaqsSection(faqs) {
    let html = `
    ...
    for (const faq of faqs) {
      ...
      <details class="border border-slate-200 rounded-lg overflow-hidden group">
        <summary class="px-6 py-4 cursor-pointer text-sm font-medium text-slate-900 hover:bg-slate-50 transition-colors list-none flex items-center justify-between">
          <span>${(0, ssr_shared_service_js_1.escapeHtml)(faq.question)}</span>
        ...
        <div class="px-6 pb-4 text-sm text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(faq.answer)}</div>
      </details>`;
    }
    html += `
    </div>
...
function renderCompareLanding(opts) {
    ...
    const breadcrumbLd = jsonLd({
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 2, name: 'Compare', item: `${ssr_shared_service_js_1.BASE_URL}/compare` },
        ],
    });
    const itemListLd = jsonLd({
        ...
        itemListElement: allEntries.map((entry, i) => ({
            ...
            url: `${ssr_shared_service_js_1.BASE_URL}/compare/${entry.slug}`,
        })),
    });
  ...
  <main id="main-content" class="max-w-5xl mx-auto px-6 py-12 lg:py-20">
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/compare`,
...
function renderCompareDetail(slug) {
    ...
    const breadcrumbLd = jsonLd({
        ...
        itemListElement: [
            { '@type': 'ListItem', position: 1, name: 'Home', item: ssr_shared_service_js_1.BASE_URL },
            { '@type': 'ListItem', position: 2, name: 'Compare', item: `${ssr_shared_service_js_1.BASE_URL}/compare` },
            { '@type': 'ListItem', position: 3, name: entry.seo.title, item: `${ssr_shared_service_js_1.BASE_URL}/compare/${entry.slug}` },
        ],
    });
    const faqLd = jsonLd({
        '@context': 'https://schema.org',
  ...
  <main id="main-content" class="max-w-5xl mx-auto px-6 py-12">
    ...
    <nav aria-label="Breadcrumb" class="mb-6">
      <ol class="flex items-center gap-2 text-sm text-slate-500">
        ...
        <li aria-hidden="true">/</li>
        <li class="text-slate-900 font-medium" aria-current="page">${(0, ssr_shared_service_js_1.escapeHtml)(entry.seo.title)}</li>
      </ol>
    </nav>

    <!-- Hero -->
    <header class="mb-10">
      <h1 class="text-3xl md:text-4xl font-display text-slate-900 mb-4">${(0, ssr_shared_service_js_1.escapeHtml)(entry.seo.title)}</h1>
      <div class="bg-indigo-50 border border-indigo-100 rounded-lg p-4 mb-6">
        <p class="text-sm font-medium text-indigo-900"><strong>TL;DR:</strong> ${(0, ssr_shared_service_js_1.escapeHtml)(entry.tldr)}</p>
      </div>
      <p class="text-slate-600 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(entry.intro)}</p>
    </header>`;
    ...
    <section class="mb-12" aria-labelledby="verdict">
      ...
      <p class="text-slate-700 leading-relaxed">${(0, ssr_shared_service_js_1.escapeHtml)(entry.verdict)}</p>
    </section>`;
    // FAQs
    ...
    return (0, ssr_shared_service_js_1.htmlShell)({
        ...
        canonicalUrl: `${ssr_shared_service_js_1.BASE_URL}/compare/${entry.slug}`,
        ogImage: `${ssr_shared_service_js_1.BASE_URL}/brand/og-default.png`,
        ogType: 'article',
        extraHead: `${breadcrumbLd}\n  ${faqLd}`,
        body: bodyContent,
...
//# sourceMappingURL=compare-ssr.service.js.map