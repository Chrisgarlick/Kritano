 ...
 */
async function createWebhook(userId, siteId, url, events) {
    // Validate URL
    let parsed;
    try {
        parsed = new URL(url);
    ...
    if (invalidEvents.length > 0) {
        throw Object.assign(new Error(`Unsupported events: ${invalidEvents.join(', ')}. Supported: ${SUPPORTED_EVENTS.join(', ')}`), { statusCode: 400 });
    }
    if (events.length === 0) {
    ...
    if (siteId) {
        const siteCheck = await index_js_1.pool.query(`SELECT id FROM sites WHERE id = $1 AND user_id = $2`, [siteId, userId]);
    ...
    const result = await index_js_1.pool.query(`INSERT INTO webhooks (user_id, site_id, url, events, secret)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`, [userId, siteId, url, events, secret]);
...
async function listWebhooks(userId) {
    const result = await index_js_1.pool.query(`SELECT id, site_id, url, events, is_active, created_at, updated_at
     FROM webhooks
     WHERE user_id = $1
...
async function deleteWebhook(userId, webhookId) {
    const result = await index_js_1.pool.query(`DELETE FROM webhooks WHERE id = $1 AND user_id = $2 RETURNING id`, [webhookId, userId]);
 ...
 */
async function listDeliveries(userId, webhookId, limit = 20) {
    // Ownership check
    const ownerCheck = await index_js_1.pool.query(`SELECT id FROM webhooks WHERE id = $1 AND user_id = $2`, [webhookId, userId]);
    if (ownerCheck.rows.length === 0) {
        throw Object.assign(new Error('Webhook not found'), { statusCode: 404 });
    }
    const result = await index_js_1.pool.query(`SELECT id, event, payload, response_status, response_body, attempts, delivered_at, created_at
     FROM webhook_deliveries
     WHERE webhook_id = $1
     ORDER BY created_at DESC
     LIMIT $2`, [webhookId, Math.min(limit, 100)]);
    return result.rows;
}
/**
 ...
 */
async function deliverEvent(event, siteId, payload) {
    try {
        // Find all active webhooks that match this event and site
        const result = await index_js_1.pool.query(`SELECT id, url, secret
       FROM webhooks
 ...
 */
async function deliverToWebhook(webhookId, url, secret, event, payload) {
    const body = JSON.stringify(payload);
    const signature = signPayload(body, secret);
    // Create delivery record
    const deliveryResult = await index_js_1.pool.query(`INSERT INTO webhook_deliveries (webhook_id, event, payload)
     VALUES ($1, $2, $3)
     RETURNING id`, [webhookId, event, payload]);
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            await index_js_1.pool.query(`UPDATE webhook_deliveries
         SET response_status = $1, response_body = $2, attempts = $3, delivered_at = NOW()
         WHERE id = $4`, [lastStatus, (lastBody || '').substring(0, 2000), attempt, deliveryId]);
         ...
         WHERE id = $3`, [(lastBody || '').substring(0, 2000), attempt, deliveryId]);
        }
