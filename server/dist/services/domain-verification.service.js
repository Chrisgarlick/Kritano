...
exports.verifyDnsTxt = verifyDnsTxt;
...
exports.isDomainVerifiedForOrg = isDomainVerifiedForOrg;
exports.revokeVerification = revokeVerification;
exports.getDomainSettingsForAudit = getDomainSettingsForAudit;
 ...
 */
async function generateVerificationToken(organizationId, domainId) {
    // Get the domain
    const domainResult = await pool.query('SELECT * FROM organization_domains WHERE id = $1 AND organization_id = $2', [domainId, organizationId]);
    ...
    if (!token) {
       ...
       WHERE id = $2`, [token, domainId]);
    }
    // Build verification instructions
    const verificationValue = `${consent_constants_js_1.VERIFICATION_TOKEN_PREFIX}${token}`;
    return {
        token,
        dns: {
            recordType: 'TXT',
            name: domain.domain,
            alternativeName: `${consent_constants_js_1.VERIFICATION_DNS_SUBDOMAIN}.${domain.domain}`,
            value: verificationValue,
        },
        file: {
            path: consent_constants_js_1.VERIFICATION_FILE_PATH,
            url: `https://${domain.domain}${consent_constants_js_1.VERIFICATION_FILE_PATH}`,
            content: token,
        },
    };
}
...
async function verifyDnsTxt(domain, expectedToken) {
    const expectedValue = `${consent_constants_js_1.VERIFICATION_TOKEN_PREFIX}${expectedToken}`;
    try {
        // Try root domain
        ...
        const subdomainResult = await queryDnsTxt(`${consent_constants_js_1.VERIFICATION_DNS_SUBDOMAIN}.${domain}`, expectedValue);
        ...
        return {
            ...
            details: `Expected TXT record with value "${expectedValue}" on ${domain} or ${consent_constants_js_1.VERIFICATION_DNS_SUBDOMAIN}.${domain}`,
        };
    }
    catch (error) {
        ...
        if (errorMessage.includes('ENOTFOUND') || errorMessage.includes('ENODATA')) {
            return {
                ...
                details: `Make sure you have added a TXT record to ${domain} or ${consent_constants_js_1.VERIFICATION_DNS_SUBDOMAIN}.${domain}`,
            };
        }
...
async function queryDnsTxt(domain, expectedValue) {
    try {
        const records = await Promise.race([
            resolveTxt(domain),
            new Promise((_, reject) => setTimeout(() => reject(new Error('ETIMEOUT')), consent_constants_js_1.VERIFICATION_LIMITS.DNS_TIMEOUT_MS)),
        ]);
 ...
 */
async function verifyFile(domain, expectedToken) {
    return await fetchVerificationFile(`https://${domain}${consent_constants_js_1.VERIFICATION_FILE_PATH}`, expectedToken);
}
async function fetchVerificationFile(url, expectedToken) {
    try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), consent_constants_js_1.VERIFICATION_LIMITS.FILE_FETCH_TIMEOUT_MS);
        ...
        if (errorMessage.includes('abort') || errorMessage.includes('timeout')) {
            return {
                ...
                details: `Could not reach ${url} within ${consent_constants_js_1.VERIFICATION_LIMITS.FILE_FETCH_TIMEOUT_MS / 1000} seconds`,
            };
        }
 ...
 */
async function attemptVerification(organizationId, domainId, method) {
    // Get the domain
    const domainResult = await pool.query('SELECT * FROM organization_domains WHERE id = $1 AND organization_id = $2', [domainId, organizationId]);
    if (domainResult.rows.length === 0) {
        throw new Error('Domain not found');
    ...
    const lastAttempt = domain.last_verification_attempt
        ...
        : null;
    if (attempts >= consent_constants_js_1.VERIFICATION_LIMITS.MAX_ATTEMPTS && lastAttempt) {
        const cooldownEnd = new Date(lastAttempt.getTime() + consent_constants_js_1.VERIFICATION_LIMITS.COOLDOWN_MS);
        if (new Date() < cooldownEnd) {
            const minutesRemaining = Math.ceil((cooldownEnd.getTime() - Date.now()) / 60000);
            throw new Error(`Too many verification attempts. Please wait ${minutesRemaining} minute(s) before trying again.`);
        }
        // Reset attempts after cooldown
        await pool.query('UPDATE organization_domains SET verification_attempts = 0 WHERE id = $1', [domainId]);
    }
 ...
 */
async function getVerificationStatus(organizationId, domainId) {
    const result = await pool.query('SELECT * FROM organization_domains WHERE id = $1 AND organization_id = $2', [domainId, organizationId]);
 ...
 */
async function isDomainVerifiedForOrg(organizationId, hostname) {
    // Normalize hostname
    const normalizedHostname = hostname.toLowerCase().replace(/^www\./, '');
    // Check for exact match or subdomain match
    const result = await pool.query(`SELECT * FROM organization_domains
     WHERE organization_id = $1 AND verified = TRUE`, [organizationId]);
 ...
 */
async function revokeVerification(organizationId, domainId) {
    await pool.query(`UPDATE organization_domains
     SET verified = FALSE,
         verified_at = NULL,
         ...
         verification_attempts = 0
     WHERE id = $1 AND organization_id = $2`, [domainId, organizationId]);
}
/**
 * Get domain bypass settings for audit worker
 * Used to apply custom settings for verified domains
 */
async function getDomainSettingsForAudit(organizationId, hostname) {
    // Normalize hostname
    const normalizedHostname = hostname.toLowerCase().replace(/^www\./, '');
    // Check for verified domain with bypass settings
    const result = await pool.query(`SELECT verified, ignore_robots_txt, rate_limit_profile, send_verification_header,
    ...
    for (const domain of result.rows) {
        const configuredDomain = domain.domain.toLowerCase().replace(/^www\./, '');
        // Check for exact match or subdomain match
        const isMatch = normalizedHostname === configuredDomain ||
