...
async function verifyPassword(userId, password) {
    ...
    if (!hash)
        return false;
    return password_service_js_1.passwordService.verify(password, hash);
}
/**
...
async function requestExport(userId) {
    ...
    const existing = await index_js_1.pool.query(`SELECT id FROM account_data_exports
    ...
    const result = await index_js_1.pool.query(`INSERT INTO account_data_exports (user_id) VALUES ($1) RETURNING id`, [userId]);
...
async function gatherUserData(userId) {
    const [userData, sites, auditJobs, auditSchedules, apiKeys, emailPrefs, consents, oauthProviders, bugReports, featureRequests, emailSends, subscriptions, organizations,] = await Promise.all([
        index_js_1.pool.query(`SELECT id, email, first_name, last_name, company_name, status, role, email_verified, created_at, last_login_at
       FROM users WHERE id = $1`, [userId]),
        index_js_1.pool.query(`SELECT id, domain, name, verified, verification_method, created_at
       FROM sites WHERE owner_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT id, target_url, target_domain, status, seo_score, accessibility_score,
       ...
       FROM audit_jobs WHERE user_id = $1 ORDER BY created_at DESC`, [userId]),
        index_js_1.pool.query(`SELECT id, site_id, name, cron_expression, frequency, enabled, created_at
       FROM audit_schedules WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT id, name, key_prefix, scopes, is_active, created_at, last_used_at, expires_at
       FROM api_keys WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT * FROM email_preferences WHERE user_id = $1`, [userId]),
        index_js_1.pool.query(`SELECT id, consent_type, consent_version, accepted_at, ip_address, created_at
       FROM user_consents WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT provider, email, name, linked_at
       FROM user_oauth_providers WHERE user_id = $1`, [userId]).catch(() => ({ rows: [] })),
        index_js_1.pool.query(`SELECT id, title, description, status, created_at
       FROM bug_reports WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT id, title, description, status, created_at
       FROM feature_requests WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT id, template_id, subject, status, sent_at, opened_at
       FROM email_sends WHERE user_id = $1 ORDER BY created_at DESC LIMIT 500`, [userId]),
        index_js_1.pool.query(`SELECT id, tier, status, trial_start, trial_end, created_at
       FROM subscriptions WHERE user_id = $1 ORDER BY created_at`, [userId]),
        index_js_1.pool.query(`SELECT o.id, o.name, o.slug, om.role as member_role, o.created_at
...
async function processExport(exportId) {
    // Mark as processing
    const exportRow = await index_js_1.pool.query(`UPDATE account_data_exports SET status = 'processing' WHERE id = $1 AND status = 'pending' RETURNING user_id, status`, [exportId]);
    if (exportRow.rows.length === 0)
        return;
    const userId = exportRow.rows[0].user_id;
    try {
        const data = await gatherUserData(userId);
        const exportDir = path_1.default.join(os_1.default.tmpdir(), 'kritano-exports');
        fs_1.default.mkdirSync(exportDir, { recursive: true });
        const filePath = path_1.default.join(exportDir, `${exportId}.zip`);
        ...
        await index_js_1.pool.query(`UPDATE account_data_exports
       SET status = 'completed', file_path = $1, file_size_bytes = $2, completed_at = NOW(), expires_at = $3
       WHERE id = $4`, [filePath, stats.size, expiresAt, exportId]);
        ...
        await index_js_1.pool.query(`UPDATE account_data_exports SET status = 'failed', error_message = $1 WHERE id = $2`, [error.message?.slice(0, 500), exportId]);
        throw error;
    }
}
/**
 ...
 */
async function getExportDownload(userId, exportId) {
    const result = await index_js_1.pool.query(`SELECT file_path, expires_at FROM account_data_exports
     WHERE id = $1 AND user_id = $2 AND status = 'completed'`, [exportId, userId]);
...
async function getLatestExport(userId) {
    const result = await index_js_1.pool.query(`SELECT id, status, created_at, completed_at, expires_at, file_size_bytes
...
async function requestAccountDeletion(userId) {
    ...
    await index_js_1.pool.query(`UPDATE users
     ...
     WHERE id = $2`, [scheduledFor, userId]);
    // Revoke all sessions
    await token_service_js_1.tokenService.revokeAllUserTokens(userId, 'account_deletion_requested');
...
async function executeAccountDeletion(userId) {
    ...
    try {
        ...
        const userIdHash = crypto_1.default.createHash('sha256').update(userId).digest('hex');
        await client.query(`INSERT INTO archived_consents (original_user_id_hash, consent_type, consent_version, accepted_at, ip_address)
       SELECT $1, consent_type, consent_version, created_at, ip_address::inet
       FROM user_consents WHERE user_id = $2`, [userIdHash, userId]);
        ...
        await client.query(`DELETE FROM organizations WHERE id IN (
...
async function runRetentionCleanup() {
    ...
    const pendingDeletions = await index_js_1.pool.query(`SELECT id FROM users
    ...
    const expiredExports = await index_js_1.pool.query(`UPDATE account_data_exports
     SET status = 'expired'
    ...
    const logResult = await index_js_1.pool.query(`DELETE FROM auth_audit_logs WHERE created_at < NOW() - INTERVAL '1 year'`);
    logsDeleted += logResult.rowCount ?? 0;
    // 4. Purge API request logs older than 90 days (per privacy policy)
    const apiLogResult = await index_js_1.pool.query(`DELETE FROM api_requests WHERE created_at < NOW() - INTERVAL '90 days'`);
    logsDeleted += apiLogResult.rowCount ?? 0;
    // 5. Purge email send logs older than 1 year (per privacy policy)
    const emailLogResult = await index_js_1.pool.query(`DELETE FROM email_sends WHERE created_at < NOW() - INTERVAL '1 year'`);
