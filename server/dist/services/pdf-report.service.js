...
exports.shutdownPdfBrowser = shutdownPdfBrowser;
...
function svgGauge(score, size = 80, strokeWidth = 8) {
    const val = score ?? 0;
...
// ── Section builders ───────────────────────────────────────────────
function buildCoverPage(audit, branding, logoDataUri, overallScore, dateStr, validScores, totalFindings, brokenLinkCount) {
...
function buildScoreOverview(validScores, scoreMap) {
...
function buildExecutiveSummary(severityCounts, findings, audit, totalFindings, brokenLinkCount) {
    // Stats row
...
function buildCategoryPage(category, catFindings, score, fixSnippets) {
    ...
    const sevOrder = { critical: 0, serious: 1, moderate: 2, minor: 3, info: 4 };
...
function buildBrokenLinksPage(brokenLinks) {
...
function buildCompliancePage(compliance) {
    ...
    const statsHtml = statCards.map(s => `<div class="compliance-stat" style="background:${s.bg}">
    ...
    const sortedClauses = [...clauses].sort((a, b) => {
        const order = { fail: 0, manual_review: 1, not_tested: 2, pass: 3 };
        return (order[a.status] ?? 4) - (order[b.status] ?? 4);
    });
    const clausesToShow = sortedClauses.slice(0, 50);
    const clauseRows = clausesToShow.map(c => `<tr class="avoid-break">
...
function buildReportMarkdown(data) {
    ...
    const validScores = Object.entries(scoreMap).filter(([, s]) => s != null);
    const overallScore = validScores.length > 0
