...
exports.registerExportTools = registerExportTools;
const zod_1 = require("zod");
const auth_js_1 = require("../auth.js");
function registerExportTools(server, pool, ctx) {
    // ── export_findings_csv ──────────────────────────────────────
    server.tool('export_findings_csv', 'Export all findings for an audit as CSV data. Returns the CSV content directly.', {
        ...
        try {
            (0, auth_js_1.requireScope)(ctx, 'exports:read');
    ...
    server.tool('export_findings_json', 'Export all findings for an audit as structured JSON data.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID'),
        category: zod_1.z.enum(['seo', 'accessibility', 'security', 'performance', 'content', 'structured-data']).optional().describe('Filter by category'),
    }, async ({ audit_id, category }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'exports:read');
            const auditCheck = await pool.query(`SELECT id, target_url, target_domain, seo_score, accessibility_score, security_score,
