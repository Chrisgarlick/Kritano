...
function registerAuditTools(server, pool, ctx) {
    // ── start_audit ──────────────────────────────────────────────
    server.tool('start_audit', 'Start a new website audit. This will create a real audit job that counts against your monthly quota. The audit runs asynchronously -- use get_audit to check progress.', {
        url: zod_1.z.string().url().describe('The URL to audit (e.g. https://example.com)'),
        max_pages: zod_1.z.number().int().min(1).max(1000).optional().describe('Maximum pages to crawl (default: 100)'),
        max_depth: zod_1.z.number().int().min(1).max(10).optional().describe('Maximum crawl depth (default: 5)'),
        checks: zod_1.z.array(zod_1.z.enum(['seo', 'accessibility', 'security', 'performance', 'content', 'file-extraction']))
            .optional()
            .describe('Which checks to run (default: all standard checks)'),
        include_mobile: zod_1.z.boolean().optional().describe('Include mobile audit pass (default: true)'),
    }, async ({ url, max_pages, max_depth, checks, include_mobile }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:write');
            // Validate URL
            let targetUrl;
            try {
            ...
            const activeAudits = await pool.query(`SELECT COUNT(*) as count FROM audit_jobs WHERE user_id = $1 AND status IN ('pending', 'discovering', 'ready', 'processing')`, [ctx.userId]);
            ...
            return {
                content: [{
                        ...
                        text: [
                            ...
                            `Status: ${(0, formatting_js_1.formatStatus)(audit.status)}`,
    ...
    // ── get_audit ────────────────────────────────────────────────
    server.tool('get_audit', 'Get the status, scores, and summary for a specific audit by ID.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID'),
    }, async ({ audit_id }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:read');
            const result = await pool.query(`SELECT * FROM audit_jobs WHERE id = $1 AND user_id = $2`, [audit_id, ctx.userId]);
            ...
            const severityCounts = await pool.query(`SELECT severity, COUNT(DISTINCT rule_id) as count FROM audit_findings WHERE audit_job_id = $1 GROUP BY severity ORDER BY
            CASE severity WHEN 'critical' THEN 1 WHEN 'serious' THEN 2 WHEN 'moderate' THEN 3 WHEN 'minor' THEN 4 ELSE 5 END`, [audit_id]);
            const sevMap = {};
            let uniqueTotal = 0;
            ...
            const lines = [
                ...
                `Status: ${(0, formatting_js_1.formatStatus)(a.status)}`,
                `Started: ${(0, formatting_js_1.formatDate)(a.started_at)} | Finished: ${(0, formatting_js_1.formatDate)(a.completed_at)}`,
                `Pages crawled: ${a.pages_crawled}/${a.max_pages}`,
            ];
            if (a.status === 'completed' || a.seo_score !== null) {
                lines.push('');
                lines.push('Scores:');
                lines.push((0, formatting_js_1.formatScoresBlock)(a));
            }
            if (uniqueTotal > 0) {
                lines.push('');
                const parts = [];
                if (sevMap.critical)
                    parts.push(`${sevMap.critical} critical`);
                if (sevMap.serious)
                    parts.push(`${sevMap.serious} serious`);
                if (sevMap.moderate)
                    parts.push(`${sevMap.moderate} moderate`);
                if (sevMap.minor)
                    parts.push(`${sevMap.minor} minor`);
                if (sevMap.info)
                    parts.push(`${sevMap.info} info`);
    ...
    // ── list_audits ──────────────────────────────────────────────
    server.tool('list_audits', 'List your recent audits with their status and scores. Supports filtering by status and domain.', {
        status: zod_1.z.enum(['pending', 'discovering', 'ready', 'processing', 'completed', 'failed', 'cancelled']).optional().describe('Filter by audit status'),
        domain: zod_1.z.string().optional().describe('Filter by target domain'),
        limit: zod_1.z.number().int().min(1).max(50).optional().describe('Max results to return (default: 10)'),
        ...
        `;
            ...
            for (const a of result.rows) {
                ...
                if (a.status === 'completed') {
                    lines.push(`         Issues: ${a.total_issues} total, ${a.critical_issues} critical | ${(0, formatting_js_1.formatDate)(a.completed_at)}`);
                }
                else {
                    lines.push(`         Created: ${(0, formatting_js_1.formatDate)(a.created_at)}`);
                }
                lines.push('');
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
    ...
    // ── get_audit_progress ───────────────────────────────────────
    server.tool('get_audit_progress', 'Get real-time progress of a running audit -- pages discovered, crawled, audited, and current URL.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID'),
    }, async ({ audit_id }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:read');
            const result = await pool.query(`SELECT status, pages_found, pages_crawled, pages_audited, current_url, max_pages,
                  total_issues, critical_issues, started_at
           FROM audit_jobs WHERE id = $1 AND user_id = $2`, [audit_id, ctx.userId]);
            if (result.rows.length === 0) {
    ...
    // ── cancel_audit ─────────────────────────────────────────────
    server.tool('cancel_audit', 'Cancel a running or pending audit. Only audits in pending/discovering/ready/processing status can be cancelled.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID to cancel'),
    }, async ({ audit_id }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:write');
            const result = await pool.query(`UPDATE audit_jobs
           SET status = 'cancelled', completed_at = NOW(), updated_at = NOW()
           WHERE id = $1 AND user_id = $2 AND status IN ('pending', 'discovering', 'ready', 'processing')
           RETURNING id, target_url, status`, [audit_id, ctx.userId]);
            if (result.rows.length === 0) {
    ...
    // ── compare_audits ───────────────────────────────────────────
    server.tool('compare_audits', 'Compare two completed audits side-by-side -- shows score deltas, new issues, and fixed issues.', {
        audit_id_before: zod_1.z.string().uuid().describe('The older audit ID (baseline)'),
        audit_id_after: zod_1.z.string().uuid().describe('The newer audit ID (comparison)'),
    }, async ({ audit_id_before, audit_id_after }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:read');
            const result = await pool.query(`SELECT * FROM audit_jobs WHERE id = ANY($1) AND user_id = $2`, [[audit_id_before, audit_id_after], ctx.userId]);
            ...
            const [beforeRules, afterRules] = await Promise.all([
                pool.query(`SELECT DISTINCT rule_id, severity FROM audit_findings WHERE audit_job_id = $1`, [audit_id_before]),
                pool.query(`SELECT DISTINCT rule_id, severity FROM audit_findings WHERE audit_job_id = $1`, [audit_id_after]),
            ]);
            ...
            const lines = [
                `Audit Comparison`,
                `Before: ${before.target_url} (${(0, formatting_js_1.formatDate)(before.completed_at)})`,
                `After:  ${after.target_url} (${(0, formatting_js_1.formatDate)(after.completed_at)})`,
                '',
                'Score Changes:',
                `  SEO:            ${(0, formatting_js_1.formatScore)(before.seo_score)} -> ${(0, formatting_js_1.formatScore)(after.seo_score)} (${delta(before.seo_score, after.seo_score)})`,
                `  Accessibility:  ${(0, formatting_js_1.formatScore)(before.accessibility_score)} -> ${(0, formatting_js_1.formatScore)(after.accessibility_score)} (${delta(before.accessibility_score, after.accessibility_score)})`,
                `  Security:       ${(0, formatting_js_1.formatScore)(before.security_score)} -> ${(0, formatting_js_1.formatScore)(after.security_score)} (${delta(before.security_score, after.security_score)})`,
                `  Performance:    ${(0, formatting_js_1.formatScore)(before.performance_score)} -> ${(0, formatting_js_1.formatScore)(after.performance_score)} (${delta(before.performance_score, after.performance_score)})`,
                `  Content:        ${(0, formatting_js_1.formatScore)(before.content_score)} -> ${(0, formatting_js_1.formatScore)(after.content_score)} (${delta(before.content_score, after.content_score)})`,
                '',
            ...
            if (fixedIssues.length > 0) {
                ...
                if (fixedIssues.length > 10)
                    lines.push(`  ... and ${fixedIssues.length - 10} more`);
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (error) {
