...
function registerComplianceTools(server, pool, ctx) {
    // ── get_compliance_status ────────────────────────────────────
    server.tool('get_compliance_status', 'Get the EAA/EN 301 549 compliance status for an audit. Maps accessibility findings to European Accessibility Act clauses.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID'),
    }, async ({ audit_id }) => {
        try {
            (0, auth_js_1.requireScope)(ctx, 'audits:read');
            const auditCheck = await pool.query(`SELECT id, target_url, wcag_version, wcag_level, accessibility_score
           FROM audit_jobs WHERE id = $1 AND user_id = $2`, [audit_id, ctx.userId]);
            ...
            const findings = await pool.query(`SELECT unnest(wcag_criteria) as criterion,
           ...
           ORDER BY criterion`, [audit_id]);
            ...
            if (hasCritical)
                complianceStatus = 'Non-compliant (critical issues)';
            else if (hasSerious)
                complianceStatus = 'Partially compliant (serious issues)';
            else if (failedCriteria.size > 0)
                complianceStatus = 'Partially compliant (minor issues)';
            const lines = [
            ...
            for (const f of findings.rows) {
                ...
                lines.push(`  ${(0, formatting_js_1.formatSeverity)(f.severity)} WCAG ${f.criterion} -> ${clause}`);
    ...
    // ── get_clause_detail ────────────────────────────────────────
    server.tool('get_clause_detail', 'Get detail for a specific WCAG criterion showing all related findings, affected pages, and the mapped EN 301 549 clause.', {
        audit_id: zod_1.z.string().uuid().describe('The audit job ID'),
        wcag_criterion: zod_1.z.string().describe('WCAG criterion (e.g. "1.1.1", "2.4.4")'),
        ...
        try {
            ...
            const result = await pool.query(`SELECT f.rule_id, f.rule_name, f.severity, f.message, f.selector, f.snippet, p.url as page_url
           ...
           ORDER BY
            ...
            for (const f of result.rows.slice(0, 20)) {
                lines.push(`${(0, formatting_js_1.formatSeverity)(f.severity)} ${f.rule_name}`);
