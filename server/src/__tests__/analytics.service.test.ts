...
// Import after mocks
// ---------------------------------------------------------------------------
import { setPool, getSiteScoreHistory } from '../services/analytics.service.js';
...
describe('Analytics Service', () => {
  ...
  // =======================================================================
  describe('calculateTrend (via getSiteScoreHistory)', () => {
    it('should report "up" trend when scores are improving', async () => {
      ...
      const result = await getSiteScoreHistory({ siteId: 'site-1', range: 'all' });

      // SEO trend should be 'up' because secondHalf avg (75) - firstHalf avg (52.5) > 2
      expect(result.summary.trends.seo).toBe('up');
    });

    it('should report "down" trend when scores are declining', async () => {
      const rows = [
      ...
      expect(result.summary.trends.accessibility).toBe('down');
    });

    it('should report "stable" trend when scores are consistent', async () => {
      const rows = [
      ...
      expect(result.summary.trends.security).toBe('stable');
    });

    it('should report "stable" when only one audit exists', async () => {
      const rows = [
      ...
      expect(result.summary.trends.seo).toBe('stable');
      expect(result.summary.trends.accessibility).toBe('stable');
    });
  });
  ...
  // =======================================================================
  describe('getSiteScoreHistory summary', () => {
    it('should calculate correct averages', async () => {
      ...
      const result = await getSiteScoreHistory({ siteId: 'site-1', range: '30d' });

      expect(result.summary.averages.seo).toBe(70);          // (60+80)/2
      expect(result.summary.averages.accessibility).toBe(85); // (80+90)/2
      expect(result.summary.totalAudits).toBe(2);
    });

    it('should handle null scores gracefully in averages', async () => {
      const rows = [
      ...
      expect(result.summary.averages.seo).toBeNull();
      // One content value is 80, the other null → average of non-null = 80
      expect(result.summary.averages.content).toBe(80);
      // Other categories still have defaults → non-null averages
      expect(result.summary.averages.accessibility).toBe(80);
    });

    it('should return empty scores array when no audits exist', async () => {
      mockQuery.mockResolvedValueOnce({ rows: [] });

      const result = await getSiteScoreHistory({ siteId: 'site-1', range: '30d' });

      expect(result.scores).toEqual([]);
      expect(result.summary.totalAudits).toBe(0);
      expect(result.summary.averages.seo).toBeNull();
    });

    it('should map database columns to ScoreDataPoint fields', async () => {
      const rows = [
        makeScoreRow({
          id: 'a1',
  ...
  // =======================================================================
  describe('edge cases', () => {
    it('should handle all null scores without crashing', async () => {
      const rows = [
        {
          id: 'a1',
      ...
      expect(result.summary.averages.seo).toBeNull();
      expect(result.summary.averages.accessibility).toBeNull();
      expect(result.summary.averages.security).toBeNull();
      expect(result.summary.averages.performance).toBeNull();
      expect(result.summary.trends.seo).toBe('stable');
    });

    it('should calculate trends correctly with mixed null/number values', async () => {
      const rows = [
