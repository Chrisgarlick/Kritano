...
async function seed() {
  ...
  try {
    ...
    const owenPassword = await passwordService.hash('dhIJT*CZc03g');
    await client.query(
      `INSERT INTO users (email, password_hash, first_name, last_name, status, email_verified, email_verified_at, beta_access)
       ...
       ON CONFLICT (email) DO UPDATE SET
         ...
         status = 'active',
         email_verified = TRUE`,
      ['owenlambert@hotmail.co.uk', owenPassword, 'Owen', 'Lambert']
    );
    ...
    const chrisPassword = await passwordService.hash('R49Setr#L7mP');
    await client.query(
      `UPDATE users SET password_hash = $1, password_changed_at = NOW() WHERE email = $2`,
      [chrisPassword, 'cgarlick94@gmail.com']
    );
