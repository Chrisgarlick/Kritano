...
import { completeCampaign, incrementCampaignStat } from '../email-campaign.service.js';
...
export function createCampaignWorker(config: CampaignWorkerConfig) {
  ...
  async function launchScheduledCampaign(campaignId: string): Promise<void> {
    ...
    const campaignResult = await pool.query(
      `SELECT c.*, t.slug as template_slug, t.subject as template_subject
    ...
    const existingSends = await pool.query(
      `SELECT COUNT(*) as cnt FROM email_sends WHERE campaign_id = $1`,
      [campaignId]
    );
    if (parseInt(existingSends.rows[0].cnt, 10) > 0) return;

    // Resolve segment using inline query (to use the worker's pool)
    const segment = campaign.segment || {};
    const conditions: string[] = ['ep.unsubscribed_all IS NOT TRUE'];
    ...
    const users = await pool.query(
      `SELECT DISTINCT u.id, u.email, u.first_name
       ...
       ${where}
       LIMIT $${paramIndex}`,
      [...values, campaign.max_recipients || 10000]
    );
    ...
    await pool.query(
      `UPDATE email_campaigns SET audience_count = $2, stats = $3, updated_at = NOW() WHERE id = $1`,
      [campaignId, users.rows.length, stats]
    );
  }

  async function processSendingCampaigns(): Promise<void> {
    // Get campaigns that are currently sending
    const result = await pool.query(
      `SELECT id, template_id, send_rate_per_second FROM email_campaigns WHERE status = 'sending'`
    );
  ...
  async function processCampaignBatch(
    ...
    const claimed = await pool.query(
      `UPDATE email_sends
       ...
       RETURNING id, template_id, user_id, to_email, subject, variables`,
    ...
    if (claimed.rows.length === 0) {
      ...
      const remaining = await pool.query(
        `SELECT COUNT(*) as cnt FROM email_sends WHERE campaign_id = $1 AND status = 'queued'`,
        [campaignId]
      );
      if (parseInt(remaining.rows[0].cnt, 10) === 0) {
        ...
        const processing = await pool.query(
          `SELECT COUNT(*) as cnt FROM email_sends WHERE campaign_id = $1 AND status = 'sending'`,
          [campaignId]
        );
    ...
    const templateResult = await pool.query(
      `SELECT slug FROM email_templates WHERE id = $1`,
      [claimed.rows[0].template_id]
    );
    ...
    for (const send of claimed.rows) {
      try {
        ...
        await incrementCampaignStat(campaignId, 'sent', 'queued');
      } catch (err) {
        // Mark individual send as failed
        await pool.query(
          `UPDATE email_sends SET status = 'failed', error_message = $2 WHERE id = $1`,
          [send.id, err instanceof Error ? err.message : 'Unknown error']
        );
        await incrementCampaignStat(campaignId, 'failed', 'queued');
