...
import {
  getDueSchedules,
  createAuditFromSchedule,
  markScheduleRun,
  pauseSchedule,
  getNextCronOccurrence,
  validateScheduleTier,
} from '../schedule.service.js';
import type { AuditSchedule, ScheduleFrequency } from '../../types/schedule.types.js';
...
export function createSchedulePoller(_config: SchedulePollerConfig) {
  let running = false;
  let timer: ReturnType<typeof setTimeout> | null = null;

  async function processSchedule(schedule: AuditSchedule): Promise<void> {
    try {
      // Re-validate tier — auto-pause if user downgraded
      const tierCheck = await validateScheduleTier(
        schedule.user_id,
      ...
      if (!tierCheck.allowed) {
        ...
        await pauseSchedule(schedule.id, `Tier downgrade: ${tierCheck.reason}`);
        return;
      }

      // Create audit job
      const result = await createAuditFromSchedule(schedule);
      ...
      if ('skipped' in result) {
        ...
        if (result.reason === 'Concurrent audit limit reached') {
          ...
          await markScheduleRun(schedule.id, 'completed', null, retryAt);
          return;
        }
        ...
        if (result.reason === 'Tier no longer allows scheduled audits') {
          await pauseSchedule(schedule.id, result.reason);
          return;
        }
      }
      ...
      if (schedule.frequency === 'biweekly' && nextRunAt) {
        const secondNext = getNextCronOccurrence(schedule.cron_expression, schedule.timezone);
      ...
      if ('auditId' in result) {
        ...
        await markScheduleRun(schedule.id, 'completed', result.auditId, adjustedNextRun);
      } else {
        await markScheduleRun(schedule.id, 'failed', null, adjustedNextRun);
        ...
        if (schedule.consecutive_failures + 1 >= maxFail) {
          ...
          await pauseSchedule(schedule.id, `Auto-paused after ${maxFail} consecutive failures`);
        }
      }
    } catch (err) {
      ...
      const nextRunAt = getNextCronOccurrence(schedule.cron_expression, schedule.timezone);
      await markScheduleRun(schedule.id, 'failed', null, nextRunAt);
  ...
  async function poll(): Promise<void> {
    ...
    try {
      const dueSchedules = await getDueSchedules(MAX_BATCH_SIZE);
