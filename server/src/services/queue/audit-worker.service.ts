...
import { createAuditEngineCoordinator, type AuditConfig } from '../audit-engines';
import { classifyError, CrawlErrorType, type CrawlError } from '../spider/error-classifier.service';
import type { AuditJob } from '../../types/audit.types';
import type { SpiderConfig, CrawlResult, SpiderEvent, DiscoveredLink } from '../../types/spider.types';
...
import {
  ...
  shouldExcludeUrl as sharedShouldExcludeUrl,
  buildAuditConfig,
} from './audit-shared';
...
const SECURITY_ERROR_TYPES = [
  ...
  CrawlErrorType.WAF_BLOCKED,
  CrawlErrorType.RATE_LIMITED,
  CrawlErrorType.IP_BLOCKED,
];
...
export class AuditWorkerService {
  ...
  constructor(config: AuditWorkerConfig) {
    ...
    this.queue = createJobQueue({ pool: config.pool });
    ...
    while (this.isRunning) {
      try {
        ...
        while (this.activeJobs.size < this.effectiveConcurrency) {
          if (!canAcceptJob()) {
            console.log(`⚠️  Memory usage above ${getMemoryThreshold()}% — skipping job claim`);
            break;
          }
  ...
  private async runAudit(job: AuditJob): Promise<void> {
    ...
    const robotsParser = createRobotsParser(DEFAULT_SPIDER_CONFIG.userAgent);
    ...
    if (isUnverifiedMode) {
      ...
      rateLimitConfig = {
        ...
        concurrentPages: UNVERIFIED_DOMAIN_LIMITS.CONCURRENT_PAGES,
      };
    } else if (domainSettings?.verified && domainSettings.rateLimitProfile) {
      // Verified domain with custom rate limit profile
      const profile = RATE_LIMIT_PROFILES[domainSettings.rateLimitProfile as RateLimitProfile];
      if (profile) {
        rateLimitConfig = {
    ...
    const spiderConfig: SpiderConfig = {
      ...
      maxPages: isUnverifiedMode
        ? Math.min(job.max_pages, UNVERIFIED_DOMAIN_LIMITS.MAX_PAGES)
        : job.max_pages,
    ...
    if (isUnverifiedMode) {
      console.log(`⚠️  Running in RESTRICTED MODE (unverified domain): Max ${UNVERIFIED_DOMAIN_LIMITS.MAX_PAGES} pages, sequential crawling, ${UNVERIFIED_DOMAIN_LIMITS.MIN_DELAY_MS}ms delay`);
      await sharedAddActivityLog(this.pool,
        job.id,
        `Running in restricted mode (unverified domain): Max ${UNVERIFIED_DOMAIN_LIMITS.MAX_PAGES} pages, sequential crawling, slower requests`,
        'info'
      );
    ...
    const spider = createSpider(spiderConfig, urlNormalizer, rateLimiter);
    await spider.initialize();

    // Build audit config using shared utility (tier checks for premium features)
    const auditConfig = await buildAuditConfig(this.pool, job);
    ...
    try {
      const tierCheck = await this.pool.query<{ available_checks: string[] }>(
        `SELECT tl.available_checks FROM tier_limits tl
         ...
         ) ut ON tl.tier = ut.tier::subscription_tier`,
        [job.site_id]
      );
    ...
    try {
      ...
      const uniqueCriticalRuleIds = new Set<string>();
      try {
        const existingFindings = await this.pool.query<{ rule_id: string; severity: string }>(
        ...
        for (const f of existingFindings.rows) {
          ...
          if (f.severity === 'critical') uniqueCriticalRuleIds.add(f.rule_id);
        }
      } catch {
        // Non-fatal
      }
      ...
      if (isSinglePageMode) {
        ...
        await sharedAddActivityLog(this.pool,job.id, 'Starting single page audit', 'info');
      } else {
        ...
        await sharedAddActivityLog(this.pool,job.id, `Starting crawl (max ${job.max_pages} pages, depth ${job.max_depth})`, 'info');
      }
      ...
      while (this.isRunning && pagesCrawled < job.max_pages) {
        ...
        if (Date.now() - auditStartTime > AUDIT_TIMEOUT_MS) {
          ...
          await sharedAddActivityLog(this.pool,job.id, 'Audit timed out after 30 minutes', 'warning');
          break;
        }

        // Fetch a batch of queue items for concurrent processing (#1)
        const batchSize = Math.min(concurrency, job.max_pages - pagesCrawled);
        const batch: { url: string; urlHash: string; depth: number }[] = [];
        ...
        const batchPromises = batch.map(async (queueItem, batchIndex) => {
          ...
          try {
            ...
            await sharedAddActivityLog(this.pool,job.id, `Crawling ${urlPath}`, 'info');
            ...
            if (findings.length > 0) {
              ...
              for (const f of findings) {
                ...
                if (f.severity === 'critical') uniqueCriticalRuleIds.add(f.ruleId);
              }
            ...
            const issueText = pageFindings > 0
              ...
              : 'No issues';
            await sharedAddActivityLog(this.pool,job.id, `✓ ${urlPath} - ${issueText}`, pageFindings > 0 ? 'warning' : 'success');
            ...
            const classifiedError = classifyError(error instanceof Error ? error : String(error));
            console.log(`   ❌ Failed: ${classifiedError.userMessage}`);
            // Log the raw error for debugging if it's an unknown error type
            if (classifiedError.type === CrawlErrorType.UNKNOWN) {
            ...
            if (classifiedError.retryable && attempts <= MAX_PAGE_RETRIES) {
              ...
              await sharedAddActivityLog(this.pool,job.id, `Retrying ${urlPath} (attempt ${attempts + 1})`, 'info');
              // Re-add to queue with lower priority for retry (forRetry=true skips duplicate check)
              await sharedAddToQueue(this.pool,job.id, queueItem.url, queueItem.depth, null, 10, true);
              ...
              await sharedAddActivityLog(this.pool,job.id, `✗ ${urlPath} - ${classifiedError.userMessage}`, 'error');
            }
          }
        });
        ...
        if (hitConsecutiveThreshold || hitRatioThreshold) {
          ...
          await sharedAddActivityLog(this.pool,
            job.id,
            `Website security protection is blocking the crawler (${totalSecurityErrors} pages blocked)`,
            'error'
          );
      ...
      if (securityBlocked) {
        ...
        throw new Error(
          `This website has security protection (e.g., Cloudflare, WAF) that is blocking automated access. ` +
      ...
      if (job.check_seo) {
        ...
        await sharedAddActivityLog(this.pool,job.id, 'Running cross-page SEO analysis...', 'info');
      ...
      await sharedAddActivityLog(this.pool,job.id, `Audit complete! ${pagesCrawled} pages crawled, ${uniqueRuleIds.size} unique issues found`, 'success');
   ...
   */
  private async getNextFromQueue(jobId: string): Promise<{ url: string; urlHash: string; depth: number } | null> {
    const result = await this.pool.query<{ url: string; url_hash: string; depth: number }>(`
   ...
   */
  private async removeFromQueue(jobId: string, urlHash: string): Promise<void> {
    await this.pool.query(`
  ...
  private async createPage(
    jobId: string,
    queueItem: { url: string; urlHash: string; depth: number }
  ): Promise<string> {
   ...
   */
  private async updatePageData(pageId: string, result: CrawlResult): Promise<void> {
    await this.pool.query(`
      UPDATE audit_pages SET
        status_code = $2,
        content_type = $3,
   ...
   */
  private async queueDiscoveredLinks(
    job: AuditJob,
    result: CrawlResult,
    currentDepth: number
  ): Promise<void> {
    const nextDepth = currentDepth + 1;
    if (nextDepth > job.max_depth) return;
    ...
    for (const link of result.links) {
      if (!link.isExternal && !link.isNoFollow) {
        ...
        if (sharedShouldExcludeUrl(link.href)) {
          continue;
        }
   ...
   */
  private async markPageFailed(pageId: string, error: CrawlError): Promise<void> {
    await this.pool.query(`
      UPDATE audit_pages SET
        crawl_status = 'failed',
        error_message = $2,
   ...
   */
  private async updatePageRetryCount(pageId: string, retryCount: number): Promise<void> {
    await this.pool.query(`
   ...
   */
  private async markPageSkipped(jobId: string, url: string, reason: string): Promise<void> {
    const urlNormalizer = createUrlNormalizer(url, false);
   ...
   */
  private async calculateFinalScores(jobId: string, config: AuditConfig): Promise<void> {
    ...
    const result = await this.pool.query<{
      ...
      avg_security: string | null;
      avg_performance: string | null;
      avg_content: string | null;
      avg_structured_data: string | null;
      page_count: string;
    }>(`
      SELECT
   ...
   */
  private async calculateErrorSummary(jobId: string): Promise<void> {
    ...
    if (pagesBlocked > 0) {
      ...
      await sharedAddActivityLog(this.pool,
        jobId,
        `${pagesBlocked} page(s) were blocked by security measures`,
        'warning'
      );
    }
  }
   ...
   */
  private async checkBrokenLinks(jobId: string): Promise<void> {
    // Get all crawled internal page URLs and their status codes
    const internalPages = await this.pool.query<{ url: string; status_code: number | null }>(
      'SELECT url, status_code FROM audit_pages WHERE audit_job_id = $1',
      [jobId]
    );
    const internalStatusMap = new Map<string, number | null>();
    ...
    const urlsToCheck = new Map<string, { sourcePageId: string; linkText: string; isExternal: boolean }[]>();
    const brokenFindings: { pageId: string; findings: Finding[] }[] = [];
    ...
    if (totalBrokenLinks > 0) {
      ...
      await sharedAddActivityLog(this.pool,jobId, `Found ${totalBrokenLinks} broken link${totalBrokenLinks !== 1 ? 's' : ''}`, 'warning');
    } else {
   ...
   */
  private async batchCheckUrls(urls: string[]): Promise<Map<string, number>> {
   ...
   */
  private async crossPageSeoChecks(jobId: string): Promise<void> {
    // Find duplicate titles
    const dupTitles = await this.pool.query<{ title: string; cnt: string; urls: string[] }>(`
    ...
    const dupDescs = await this.pool.query<{ meta_description: string; cnt: string; urls: string[] }>(`
    ...
    const crawledPages = await this.pool.query<{ id: string; url: string; depth: number }>(`
   ...
   */
  private async cleanupCrawlQueue(jobId: string): Promise<void> {
    const result = await this.pool.query(
      'DELETE FROM crawl_queue WHERE audit_job_id = $1',
      [jobId]
    );
 ...
 */
export function createAuditWorker(config: AuditWorkerConfig): AuditWorkerService {
