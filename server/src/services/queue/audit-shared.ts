 ...
 */
export async function buildAuditConfig(pool: Pool, job: AuditJob): Promise<AuditConfig> {
  let checkEeat = false;
  let checkAeo = false;
  let checkFileExtraction = false;

  try {
    const tierCheck = await pool.query<{ available_checks: string[] }>(
