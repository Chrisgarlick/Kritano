...
export interface CrawlError {
  type: CrawlErrorType;
  category: CrawlErrorCategory; // Error category for grouping
  message: string;         // Technical error message
  userMessage: string;     // User-friendly message
  suggestion: string;      // Actionable suggestion
  retryable: boolean;      // Whether retrying might help
  statusCode?: number;     // HTTP status code if applicable
  isSecurityRelated: boolean;  // Whether this is a security/bot detection issue
}
 ...
 */
export function getErrorTypeLabel(type: CrawlErrorType): string {
  const labels: Record<CrawlErrorType, string> = {
 ...
 */
export function hasSecurityBlockingErrors(errors: CrawlError[]): boolean {
 ...
 */
export function getSecurityBlockingSummary(errors: CrawlError[]): string | null {
  const securityErrors = errors.filter(e => e.isSecurityRelated);
  if (securityErrors.length === 0) return null;
