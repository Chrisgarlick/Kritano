 ...
 */
export async function detectSPA(page: Page): Promise<SPADetectionResult> {
  return page.evaluate(() => {
    ...
    if (w.__NEXT_DATA__ || d.querySelector('#__next')) {
      return { isSPA: true, framework: 'Next.js', confidence: 'high' as const };
    }
    // Nuxt.js
    if (w.__NUXT__ || d.querySelector('#__nuxt')) {
      return { isSPA: true, framework: 'Nuxt.js', confidence: 'high' as const };
    }
    // React
    if (w.React || d.querySelector('[data-reactroot]')) {
      return { isSPA: true, framework: 'React', confidence: 'high' as const };
    }
    // Vue.js
    if (w.Vue || d.querySelector('[data-v-app]') || d.querySelector('#app[data-server-rendered]')) {
      return { isSPA: true, framework: 'Vue.js', confidence: 'high' as const };
    }
    // Angular
    if (w.angular || w.ng || d.querySelector('[ng-app]') || d.querySelector('[ng-version]')) {
      return { isSPA: true, framework: 'Angular', confidence: 'high' as const };
    }
    // Svelte
    if (d.querySelector('[class*="svelte-"]')) {
      return { isSPA: true, framework: 'Svelte', confidence: 'high' as const };
    }
    // Gatsby
    if (d.querySelector('#___gatsby')) {
      return { isSPA: true, framework: 'Gatsby', confidence: 'high' as const };
    }
    // Remix
    if (w.__remixContext) {
      return { isSPA: true, framework: 'Remix', confidence: 'high' as const };
    }
    // Astro
    if (d.querySelector('[data-astro-cid]') || d.querySelector('astro-island')) {
      return { isSPA: true, framework: 'Astro', confidence: 'high' as const };
    }
    ...
    if (hasRootContainer && hasMinimalHTML) {
      return { isSPA: true, framework: null, confidence: 'medium' as const };
    }

    if (hasPushState && hasHashRouter) {
      return { isSPA: true, framework: null, confidence: 'low' as const };
    }

    return { isSPA: false, framework: null, confidence: 'high' as const };
  });
}
 ...
 */
export async function waitForLoadingIndicators(
  page: Page,
  options: {
    selectors?: string[];
    timeoutMs?: number;
 ...
 * Note: Uses string-based function to avoid esbuild __name helper issues
 */
export async function waitForLazyContent(page: Page): Promise<void> {
...
export async function smartWait(
  ...
  const {
    waitForNetworkIdle: doNetworkWait = true,
    waitForDOMStability: doDOMWait = true,
    waitForLoadingIndicators: doLoadingWait = true,
    waitForImages: doImageWait = false,
    detectSPA: doSPADetect = true,
    maxWaitMs = 15000,
  } = options;

  const startTime = Date.now();
  let spaResult: SPADetectionResult = { isSPA: false, framework: null, confidence: 'high' };
...
export async function waitForSPAContent(
  ...
  if (framework === 'Next.js') {
    ...
    try {
      await page.waitForFunction(
        () => (window as any).$nuxt?.$loading?.loading === false,
        { timeout: maxWaitMs }
      );
    } catch {
      // Continue
