...
import { getSubscription, checkDomainLimit } from './organization.service.js';
...
export async function addDomain(
  ...
  // Check domain limit
  const canAdd = await checkDomainLimit(organizationId);
  if (!canAdd) {
  ...
  if (isFree) {
    const existingDomains = await pool.query(
      'SELECT id FROM organization_domains WHERE organization_id = $1',
      [organizationId]
    );
...
export async function getDomainById(
  ...
  const result = await pool.query<OrganizationDomain>(
    'SELECT * FROM organization_domains WHERE id = $1',
    [domainId]
  );
 ...
 */
export async function lockDomainForFreeTier(
  organizationId: string,
  domain: string
): Promise<void> {
  const subscription = await getSubscription(organizationId);
  ...
  await pool.query(
    `UPDATE organization_domains
     ...
     WHERE organization_id = $2 AND domain = $3 AND status IN ('pending', 'active')`,
    [endOfMonth, organizationId, normalizeDomain(domain)]
  );
}
 ...
 */
export async function requestDomainChange(
  organizationId: string,
  ...
  const result = await pool.query<OrganizationDomain>(
    `UPDATE organization_domains
     ...
     WHERE id = $2
     RETURNING *`,
    [normalizedNewDomain, currentDomainId]
  );
 ...
 */
export async function cancelDomainChange(
  domainId: string
): Promise<OrganizationDomain> {
  const result = await pool.query<OrganizationDomain>(
    `UPDATE organization_domains
 ...
 */
export async function processMonthlyDomainResets(): Promise<number> {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    // Find all FREE tier domains with pending changes
    const pendingChanges = await client.query<OrganizationDomain>(
      `SELECT od.* FROM organization_domains od
    ...
    for (const domain of pendingChanges.rows) {
      ...
      await client.query(
        `UPDATE organization_domains
         ...
         WHERE id = $2`,
        [domain.pending_domain, domain.id]
      );
      processed++;
    }
 ...
 */
export async function isUrlAllowedForAudit(
  organizationId: string,
  url: string
): Promise<{ allowed: boolean; reason?: string; matchedDomain?: OrganizationDomain }> {
