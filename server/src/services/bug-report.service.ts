...
export const bugReportService = {
  ...
  async listByUser(userId: string, options: ListOptions): Promise<PaginatedResult<BugReport>> {
    ...
    const result = await pool.query<BugReport>(query, params);
  ...
  async getWithComments(id: string): Promise<BugReportWithComments | null> {
    ...
    const comments = await pool.query<BugReportComment>(
   ...
   */
  async addComment(data: AddCommentData): Promise<BugReportComment> {
  ...
  async listAll(options: AdminListOptions): Promise<PaginatedResult<BugReport>> {
    ...
    const countQuery = query.replace(
      'SELECT br.*, u.email as reporter_email',
      'SELECT COUNT(*)'
    );
    const countResult = await pool.query(countQuery, params);
    const total = parseInt(countResult.rows[0].count, 10);
   ...
   */
  async update(id: string, data: UpdateBugReportData): Promise<BugReport | null> {
  ...
  async getStats(): Promise<BugReportStats> {
    const result = await pool.query<BugReportStats>(`
      SELECT
