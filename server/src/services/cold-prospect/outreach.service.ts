 ...
 */
export async function queueOutreachBatch(limit?: number): Promise<{ queued: number }> {
  // Get daily limit from settings
  const settingResult = await pool.query(
    `SELECT value FROM system_settings WHERE key = 'cold_prospect_daily_email_limit'`
  );
  ...
  const todayResult = await pool.query(
    `SELECT COUNT(*) as count FROM cold_prospect_sends
...
export async function processOutreachQueue(batchSize: number = 10): Promise<{ sent: number; failed: number; drafted: number }> {
  ...
  const result = await pool.query(
    `UPDATE cold_prospect_sends
     ...
     RETURNING id, prospect_id, template_slug, to_email, subject`,
    [batchSize]
  );
  ...
  for (const send of result.rows) {
    try {
      ...
      const prospectResult = await pool.query(
        `SELECT domain, contact_name, contact_email, quality_score
         FROM cold_prospects WHERE id = $1`,
        [send.prospect_id]
      );
      ...
      const templateResult = await pool.query(
        `SELECT id, subject, blocks, branding_mode FROM email_templates
      ...
      // Mark as sent
      await pool.query(
        `UPDATE cold_prospect_sends SET status = 'sent', sent_at = NOW(), updated_at = NOW()
         WHERE id = $1`,
        [send.id]
      );
      ...
      await pool.query(
        `UPDATE cold_prospects SET status = 'contacted', email_sent_at = NOW()
         WHERE id = $1`,
        [send.prospect_id]
      );
...
export async function getSendHistory(
  ...
  const [dataResult, countResult] = await Promise.all([
    pool.query(
      `SELECT cps.*, cp.domain, cp.contact_name
 ...
 */
export async function processUnsubscribe(email: string, prospectId?: string): Promise<void> {
  await pool.query(
    `INSERT INTO cold_prospect_unsubscribes (email, prospect_id)
     VALUES ($1, $2)
     ON CONFLICT (email) DO NOTHING`,
    [email, prospectId || null]
  );

  // Also update prospect status if we have the ID
  if (prospectId) {
    await pool.query(
      `UPDATE cold_prospects SET excluded_at = NOW(), exclusion_reason = 'unsubscribed'
...
export function generateUnsubscribeToken(email: string, prospectId: string): string {
  return jwt.sign({ email, prospectId }, JWT_SECRET, { expiresIn: '365d' });
}

export function verifyUnsubscribeToken(token: string): { email: string; prospectId: string } | null {
  try {
    const decoded = jwt.verify(token, JWT_SECRET) as { email: string; prospectId: string };
    return decoded;
  } catch {
    return null;
  }
...
async function markSendFailed(sendId: string, error: string): Promise<void> {
  await pool.query(
    `UPDATE cold_prospect_sends SET status = 'failed', error_message = $2, updated_at = NOW()
     WHERE id = $1`,
    [sendId, error]
  );
}
...
async function sendColdEmail(
  ...
  const templateResult = await pool.query(
    `SELECT blocks, compiled_html, compiled_at FROM email_templates WHERE slug = $1`,
    [templateSlug]
  );
  if (templateResult.rows.length === 0) {
 ...
 */
function buildSimpleHtml(blocks: Array<{ type: string; content?: string; label?: string; href?: string; companyName?: string; backgroundColor?: string }>): string {
