...
function getDateRangeFilter(range: TimeRange): Date {
  const now = new Date();
  switch (range) {
    case '7d':
...
function calculateTrend(values: (number | null)[]): 'up' | 'down' | 'stable' {
...
export async function getSiteScoreHistory(options: ScoreHistoryOptions): Promise<ScoreHistory> {
  ...
  const result = await pool.query<{
    ...
    [siteId, dateFilter, endDate]
  );

  const scores: ScoreDataPoint[] = result.rows.map(row => ({
    auditId: row.id,
  ...
  // Calculate summary
  const validScores = {
    seo: scores.map(s => s.seo).filter((v): v is number => v !== null),
    accessibility: scores.map(s => s.accessibility).filter((v): v is number => v !== null),
    security: scores.map(s => s.security).filter((v): v is number => v !== null),
...
export async function getIssueTrends(options: IssueTrendOptions): Promise<IssueTrends> {
  ...
  const result = await pool.query<{
    ...
    ORDER BY period ASC`,
    [siteId, dateFilter, truncation]
  );

  // Group results by period
  const periodMap = new Map<string, IssueTrendPoint>();
  ...
  for (const row of result.rows) {
    ...
    if (row.severity in point.bySeverity) {
      point.bySeverity[row.severity as keyof typeof point.bySeverity] += count;
    }

    if (row.category in point.byCategory) {
      point.byCategory[row.category as keyof typeof point.byCategory] += count;
    }
  }
...
export async function compareAudits(auditIds: string[]): Promise<AuditComparison> {
  ...
  const severityResult = await pool.query<{
    ...
    GROUP BY audit_job_id, severity`,
    [auditIds]
  );

  const severityByAudit = new Map<string, { critical: number; serious: number; moderate: number; minor: number }>();
  ...
  const audits: AuditSummary[] = auditsResult.rows.map(row => {
  ...
  const issuesResult = await pool.query<{
    ...
    `SELECT DISTINCT ON (audit_job_id, rule_id)
      audit_job_id,
      rule_id,
      rule_name,
      category,
  ...
  const severityRank: Record<string, number> = { critical: 1, serious: 2, moderate: 3, minor: 4 };
  const issuesByRule = new Map<string, { ruleName: string; category: string; severity: string; presentIn: Set<string> }>();
  ...
  // Categorize issues
  const commonIssues: AuditComparison['comparison']['commonIssues'] = [];
  const resolvedIssues: AuditComparison['comparison']['resolvedIssues'] = [];
  const newIssues: AuditComparison['comparison']['newIssues'] = [];
...
export async function getOrganizationOverview(organizationId: string): Promise<OrgAnalytics> {
  ...
  const validScores = {
    seo: sites.map(s => s.latestScores.seo).filter((v): v is number => v !== null),
    accessibility: sites.map(s => s.latestScores.accessibility).filter((v): v is number => v !== null),
    security: sites.map(s => s.latestScores.security).filter((v): v is number => v !== null),
...
export async function compareSites(siteIds: string[]): Promise<SiteComparison> {
  ...
  const sites: SiteComparisonEntry[] = result.rows.map(row => ({
    id: row.id,
    name: row.name,
    domain: row.domain,
...
async function computeUserOverview(userId: string): Promise<UserOverview> {
  ...
  const auditsCountResult = await pool.query<{ count: string }>(
    `SELECT COUNT(*)::text as count FROM audit_jobs WHERE site_id = ANY($1) AND status = 'completed'`,
    [siteIds]
  );
  const totalAudits = parseInt(auditsCountResult.rows[0]?.count || '0', 10);

  // Get latest scores for each site
  const latestScoresResult = await pool.query<{
    site_id: string;
    seo_score: number | null;
    accessibility_score: number | null;
    security_score: number | null;
  ...
  const allScores = {
    seo: latestScoresResult.rows.map(r => r.seo_score).filter((v): v is number => v !== null),
    accessibility: latestScoresResult.rows.map(r => r.accessibility_score).filter((v): v is number => v !== null),
    security: latestScoresResult.rows.map(r => r.security_score).filter((v): v is number => v !== null),
  ...
  const recentActivityResult = await pool.query<{
    ...
    `SELECT aj.id, aj.site_id, aj.completed_at, aj.seo_score, aj.accessibility_score, aj.security_score,
            aj.performance_score,
  ...
  const recentActivity: UserOverview['recentActivity'] = recentActivityResult.rows.map(row => {
    ...
    let scanType: 'single-page' | 'quick-scan' | 'full-audit' | 'accessibility' | 'custom';
...
export async function getUrlScoreHistory(options: UrlScoreHistoryOptions): Promise<ScoreHistory> {
...
export async function getUrlAnalytics(urlId: string, siteId: string): Promise<UrlAnalytics> {
  ...
  const siteAvgResult = await pool.query<{
    avg_seo: number | null;
    avg_accessibility: number | null;
    avg_security: number | null;
    avg_performance: number | null;
    ...
    FROM site_urls
    WHERE site_id = $1 AND last_audited_at IS NOT NULL`,
    [siteId]
  );
  ...
  function calcDiff(urlScore: number | null, siteScore: number | null): number | null {
    if (urlScore === null || siteScore === null) return null;
  ...
  const recentAuditsResult = await pool.query<{
    audit_id: string;
    completed_at: Date;
    seo_score: number | null;
    accessibility_score: number | null;
  ...
  const recentAudits: UrlAnalytics['recentAudits'] = recentAuditsResult.rows.map(row => ({
    id: row.audit_id,
    completedAt: row.completed_at.toISOString(),
    scores: {
      seo: row.seo_score,
...
export async function getUserAuditedUrls(
  userId: string,
  search?: string,
  limit: number = 50
): Promise<UserAuditedUrl[]> {
  const params: any[] = [userId, Math.min(limit, 100)];
  let searchClause = '';
...
async function getUrlPageSnapshot(
  siteId: string,
  urlId: string
): Promise<UrlPageSnapshot> {
  ...
  const urlResult = await pool.query<{
    ...
    `SELECT su.id, su.url, su.url_path, s.id as site_id, s.name as site_name, s.domain as site_domain
     FROM site_urls su
     JOIN sites s ON s.id = su.site_id
     WHERE su.id = $1 AND su.site_id = $2`,
    [urlId, siteId]
  );
  ...
  const pageResult = await pool.query<{
    ...
    accessibility_issues: number | null;
    security_issues: number | null;
    performance_issues: number | null;
    content_issues: number | null;
...
async function getUrlFindingsDiff(
  pageIdA: string,
  pageIdB: string
): Promise<UrlFindingsDiff> {
  const result = await pool.query<{
    ...
    `SELECT af.audit_page_id, af.rule_id, af.rule_name, af.category, af.severity, af.message, af.recommendation
     FROM audit_findings af
  ...
  const findingsA: Map<string, UrlFindingItem> = new Map();
  const findingsB: Map<string, UrlFindingItem> = new Map();

  for (const row of result.rows) {
    const item: UrlFindingItem = {
      ruleId: row.rule_id,
      ruleName: row.rule_name,
      category: row.category,
  ...
  const uniqueToA: UrlFindingItem[] = [];
  const uniqueToB: UrlFindingItem[] = [];
  const shared: UrlFindingItem[] = [];
  ...
  function countSeverities(items: UrlFindingItem[]) {
    return {
...
function generateUrlInsights(a: UrlPageSnapshot, b: UrlPageSnapshot): UrlComparisonInsight[] {
  const insights: UrlComparisonInsight[] = [];

  // Helper to determine winner
  function winner(aVal: number | null, bVal: number | null): 'a' | 'b' | 'tie' {
    if (aVal === null && bVal === null) return 'tie';
    if (aVal === null) return 'b';
    if (bVal === null) return 'a';
    if (Math.abs(aVal - bVal) < 1) return 'tie';
    return bVal > aVal ? 'b' : 'a';
  }

  function severity(delta: number): 'high' | 'medium' | 'low' {
  ...
  // Main scores
  const scoreCategories: { key: keyof typeof a.scores; label: string }[] = [
    { key: 'seo', label: 'SEO' },
  ...
  // Content subscores
  const contentSubs: { key: keyof typeof a.contentSubscores; label: string }[] = [
  ...
  // E-E-A-T pillars
  const eeatPillars: { key: keyof Pick<typeof a.eeat, 'experience' | 'expertise' | 'authoritativeness' | 'trustworthiness'>; label: string }[] = [
  ...
  const tierRank: Record<string, number> = { 'ghost-content': 0, 'standard-web': 1, 'expert-verified': 2 };
  ...
  // AEO comparison
  const aeoPillars: { key: keyof Pick<typeof a.aeo, 'nuggetScore' | 'factualDensity' | 'sourceAuthority'>; label: string }[] = [
  ...
  // AEO tier comparison
  const aeoTierRank: Record<string, number> = { 'ignored': 0, 'general-reference': 1, 'primary-source': 2 };
  ...
  const issueCats: { key: keyof typeof a.issueCountByCategory; label: string }[] = [
...
export async function compareUrls(
  urlSpecs: [{ siteId: string; urlId: string }, { siteId: string; urlId: string }]
  ...
  const scoreKeys = ['seo', 'accessibility', 'security', 'performance', 'content', 'structuredData'] as const;
