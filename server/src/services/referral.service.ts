...
export async function getConfig(): Promise<ReferralConfig> {
  ...
  return {
    ...
    rewards: (configMap.rewards as ReferralConfig['rewards']) || {
...
export async function resolveReferralCode(code: string): Promise<{ userId: string; email: string } | null> {
  const result = await pool.query(
    'SELECT id, email FROM users WHERE referral_code = $1 AND status != $2',
    [code, 'deleted']
  );
...
export async function createReferral(
  ...
  const emails = users.rows.map((r: { email: string }) => r.email.toLowerCase());
  if (emails.length === 2 && emails[0] === emails[1]) return null;
  ...
  if (parseInt(monthCount.rows[0].cnt, 10) >= config.maxReferralsPerMonth) return null;
  ...
  const referrerIpResult = await pool.query(
    'SELECT last_login_ip FROM users WHERE id = $1',
    [referrerId]
  );

  const result = await pool.query(
    `INSERT INTO referrals (referrer_id, referred_id, referral_code, referrer_ip, referred_ip, referrer_tier)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (referred_id) DO NOTHING
     RETURNING *`,
    [referrerId, referredId, code, referrerIpResult.rows[0]?.last_login_ip || null, referredIp || null, referrerTier]
  );
  ...
  await pool.query(
    'UPDATE users SET referred_by_code = $1, referred_by_user_id = $2 WHERE id = $3',
...
export async function checkAndQualifyReferral(referredUserId: string): Promise<void> {
  // Find pending referral for this user
  const refResult = await pool.query(
    `SELECT r.*, u.email_verified, u.first_name, u.last_name, u.email as referred_email
     FROM referrals r
  ...
  if (!referral.email_verified_at) {
    await pool.query(
      'UPDATE referrals SET email_verified_at = NOW(), updated_at = NOW() WHERE id = $1',
      [referral.id]
    );
  }
...
async function applyRewards(referralId: string): Promise<void> {
  ...
  try {
    ...
    // Credit referrer
    await client.query(
      'UPDATE users SET referral_bonus_audits = referral_bonus_audits + $1 WHERE id = $2',
      [referrerBonus, referral.referrer_id]
    );
    const referrerBalance = await client.query(
      'SELECT referral_bonus_audits FROM users WHERE id = $1',
      [referral.referrer_id]
    );
    await client.query(
      `INSERT INTO referral_rewards (user_id, referral_id, type, amount, balance_after, description)
       VALUES ($1, $2, 'bonus_audits', $3, $4, $5)`,
      [referral.referrer_id, referralId, referrerBonus, referrerBalance.rows[0].referral_bonus_audits,
    ...
    await client.query(
      'UPDATE users SET referral_bonus_audits = referral_bonus_audits + $1 WHERE id = $2',
      [referredBonus, referral.referred_id]
    );
    const referredBalance = await client.query(
      'SELECT referral_bonus_audits FROM users WHERE id = $1',
    ...
    await client.query(
      `INSERT INTO referral_rewards (user_id, referral_id, type, amount, balance_after, description)
       VALUES ($1, $2, 'bonus_audits', $3, $4, $5)`,
      [referral.referred_id, referralId, referredBonus, referredBalance.rows[0].referral_bonus_audits,
    ...
    await client.query(
      `UPDATE referrals SET
         ...
         updated_at = NOW()
       WHERE id = $3`,
      [referrerBonus, referredBonus, referralId]
    );

    await client.query('COMMIT');

    // Send notification emails (non-blocking)
    sendReferralEmails(referral, referrerBonus, referredBonus).catch(err =>
      console.error('Failed to send referral emails:', err)
    );

    // Check milestones
    checkMilestones(referral.referrer_id, config).catch(err =>
      console.error('Failed to check referral milestones:', err)
    );
  } catch (err) {
...
async function sendReferralEmails(referral: Referral & Record<string, unknown>, referrerBonus: number, referredBonus: number): Promise<void> {
  // Get referrer info
  const referrerResult = await pool.query(
    'SELECT first_name, email, referral_bonus_audits FROM users WHERE id = $1',
    [referral.referrer_id]
  );
...
async function checkMilestones(referrerId: string, config: ReferralConfig): Promise<void> {
  ...
  const existing = await pool.query(
    `SELECT id FROM referral_rewards
     WHERE user_id = $1 AND type = 'tier_upgrade' AND description LIKE $2`,
    [referrerId, `%${milestoneKey} referral milestone%`]
  );
  ...
  await pool.query(
    ...
    [referrerId, milestone.tier, trialEnd.toISOString()]
  );

  // Log reward
  const balance = await pool.query('SELECT referral_bonus_audits FROM users WHERE id = $1', [referrerId]);
  await pool.query(
    `INSERT INTO referral_rewards (user_id, type, amount, balance_after, description)
     VALUES ($1, 'tier_upgrade', 0, $2, $3)`,
    [referrerId, balance.rows[0].referral_bonus_audits,
...
export async function consumeBonusAudit(userId: string): Promise<boolean> {
  const result = await pool.query(
    `UPDATE users SET referral_bonus_audits = referral_bonus_audits - 1
     WHERE id = $1 AND referral_bonus_audits > 0
     RETURNING referral_bonus_audits`,
    [userId]
  );
  ...
  await pool.query(
    `INSERT INTO referral_rewards (user_id, type, amount, balance_after, description)
     VALUES ($1, 'consumed', -1, $2, 'Bonus audit consumed')`,
    [userId, result.rows[0].referral_bonus_audits]
  );
  return true;
}
...
export async function getUserReferrals(
  ...
  const result = await pool.query(
    `SELECT r.*, u.email as referred_email, u.first_name || ' ' || u.last_name as referred_name
     FROM referrals r
     JOIN users u ON u.id = r.referred_id
...
export async function sendInviteEmails(
  ...
  const userResult = await pool.query(
    'SELECT first_name, last_name, email, referral_code FROM users WHERE id = $1',
    [userId]
  );
  const user = userResult.rows[0];
...
export async function adminGetStats(): Promise<AdminReferralStats> {
  const stats = await pool.query(`
    SELECT
      COUNT(*) as total,
      COUNT(*) FILTER (WHERE status = 'pending') as pending,
  ...
  const totalBonusResult = await pool.query(
...
export async function adminVoidReferral(referralId: string, reason: string, adminId: string): Promise<Referral> {
  const refResult = await pool.query('SELECT * FROM referrals WHERE id = $1', [referralId]);
  ...
  try {
    ...
    if (referral.status === 'rewarded') {
      ...
      if (referral.referrer_reward_value) {
        await client.query(
          'UPDATE users SET referral_bonus_audits = GREATEST(0, referral_bonus_audits - $1) WHERE id = $2',
          [referral.referrer_reward_value, referral.referrer_id]
        );
        const referrerBalance = await client.query(
          'SELECT referral_bonus_audits FROM users WHERE id = $1',
          [referral.referrer_id]
        ...
        await client.query(
          ...
          [referral.referrer_id, referralId, -referral.referrer_reward_value,
      ...
      if (referral.referred_reward_value) {
        await client.query(
          'UPDATE users SET referral_bonus_audits = GREATEST(0, referral_bonus_audits - $1) WHERE id = $2',
          [referral.referred_reward_value, referral.referred_id]
        );
        const referredBalance = await client.query(
          'SELECT referral_bonus_audits FROM users WHERE id = $1',
          [referral.referred_id]
        ...
        await client.query(
          `INSERT INTO referral_rewards (user_id, referral_id, type, amount, balance_after, description)
           VALUES ($1, $2, 'admin_adjustment', $3, $4, $5)`,
          [referral.referred_id, referralId, -referral.referred_reward_value,
    ...
    const updated = await client.query(
      `UPDATE referrals SET status = 'voided', voided_at = NOW(), void_reason = $1, updated_at = NOW()
