...
function discoverCssFile(): string | null {
  const candidates = [
    path.resolve(process.cwd(), 'client', 'dist', 'assets'),
    path.resolve(process.cwd(), '..', 'client', 'dist', 'assets'),
  ...
  for (const dir of candidates) {
    try {
      ...
      if (cssFile) return `/assets/${cssFile}`;
...
function renderBlock(block: ContentBlock): string {
  ...
  switch (block.type) {
    ...
    case 'callout': {
      ...
      const colours: Record<string, { bg: string; border: string; titleColor: string; label: string }> = {
    ...
    case 'code': {
      ...
      const code = escapeHtml((props.code as string) || '');
    ...
    case 'cta': {
      const text = escapeHtml((props.text as string) || 'Learn more');
    ...
    case 'stat_highlight': {
      const stat = escapeHtml((props.stat as string) || '');
    ...
    case 'audit_link': {
      const ruleId = escapeHtml((props.ruleId as string) || '');
      const customText = escapeHtml((props.customText as string) || `View rule: ${ruleId}`);
...
function buildStructuredData(post: BlogPost): object[] {
  ...
  if (post.schema_type === 'howto') {
    const steps: { name: string; textParts: string[]; image?: string }[] = [];
    ...
    for (const block of post.content) {
      ...
      if (block.type === 'heading') {
        ...
        currentStep = { name: p.text as string, textParts: [] };
      } else if (currentStep) {
        if (block.type === 'text') currentStep.textParts.push(p.content as string);
        else if (block.type === 'image' && !currentStep.image) currentStep.image = p.src as string;
      }
    }
    if (currentStep) steps.push(currentStep);
  ...
  if (post.schema_type === 'claim_review' && post.schema_claim_reviewed) {
    const ratingMap: Record<string, { value: number; alternateName: string }> = {
  ...
  for (const block of post.content) {
    if (block.type === 'embed' && block.props.url) {
      schemas.push({
        ...
        name: post.title,
        description: post.seo_description || post.excerpt,
        embedUrl: block.props.url as string,
  ...
  return `<header role="banner" class="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
    ...
    <nav aria-label="Main navigation" class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <div class="hidden md:flex items-center gap-6">
        <a href="/services" class="text-[15px] font-medium text-slate-600 hover:text-indigo-600 transition-colors">Services</a>
        <a href="/pricing" class="text-[15px] font-medium text-slate-600 hover:text-indigo-600 transition-colors">Pricing</a>
        <a href="/about" class="text-[15px] font-medium text-slate-600 hover:text-indigo-600 transition-colors">About</a>
        <a href="/blog" class="text-[15px] font-medium text-indigo-600" aria-current="page">Blog</a>
        <a href="/contact" class="text-[15px] font-medium text-slate-600 hover:text-indigo-600 transition-colors">Contact</a>
        <a href="/docs" class="text-[15px] font-medium text-slate-600 hover:text-indigo-600 transition-colors">API Docs</a>
      </div>
      <div class="hidden md:flex items-center gap-6">
    ...
    <div class="ssr-mobile-menu md:hidden border-t border-slate-100 bg-white">
      <div class="px-6 py-4 space-y-1">
        <a href="/services" class="block py-3 text-base font-medium text-slate-600 hover:text-slate-900 transition-colors">Services</a>
        <a href="/pricing" class="block py-3 text-base font-medium text-slate-600 hover:text-slate-900 transition-colors">Pricing</a>
        <a href="/about" class="block py-3 text-base font-medium text-slate-600 hover:text-slate-900 transition-colors">About</a>
        <a href="/blog" class="block py-3 text-base font-medium text-indigo-600" aria-current="page">Blog</a>
        <a href="/contact" class="block py-3 text-base font-medium text-slate-600 hover:text-slate-900 transition-colors">Contact</a>
        <a href="/docs" class="block py-3 text-base font-medium text-slate-600 hover:text-slate-900 transition-colors">API Docs</a>
...
function renderAuthorBio(): string {
...
export function renderBlogListing(posts: PostSummary[], total: number, page: number, totalPages: number, category?: string, tag?: string): string {
  ...
  let extraHead = '';
  if (page > 1) extraHead += `<link rel="prev" href="${BASE_URL}/blog?page=${page - 1}${category ? `&category=${encodeURIComponent(category)}` : ''}${tag ? `&tag=${encodeURIComponent(tag)}` : ''}" />`;
  if (page < totalPages) extraHead += `\n  <link rel="next" href="${BASE_URL}/blog?page=${page + 1}${category ? `&category=${encodeURIComponent(category)}` : ''}${tag ? `&tag=${encodeURIComponent(tag)}` : ''}" />`;
