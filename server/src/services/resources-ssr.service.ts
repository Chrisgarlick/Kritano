...
const FORMAT_LABELS: Record<ResourceFormat, string> = {
  md: 'Markdown',
  pdf: 'PDF',
  html: 'HTML',
  docx: 'Word (DOCX)',
...
const FORMAT_DESCRIPTIONS: Record<ResourceFormat, string> = {
...
function isTypesetEnabled(): boolean {
  return process.env.TYPESET_ENABLED === 'true';
}

/** True when this format needs typeset to be rendered. */
function requiresTypeset(format: ResourceFormat): boolean {
  return format !== 'md';
}

// ── Resource card (used on the list page) ───────────────────────────

function renderResourceCard(r: ResourceSummary): string {
  const formats = r.formats
    .map(
      (f) =>
        `<span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-md uppercase tracking-wider">${escapeHtml(f)}</span>`
    )
    .join('\n        ');

  return `<a href="/resources/${escapeHtml(r.slug)}" class="block group bg-white border border-slate-200 rounded-xl p-6 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all">
    ...
    <h2 class="font-display text-2xl text-slate-900 leading-tight mb-3 group-hover:text-indigo-600 transition-colors">${escapeHtml(r.title)}</h2>
    <p class="text-sm text-slate-600 leading-relaxed mb-5">${escapeHtml(r.hook)}</p>
...
export function renderResourcesList(resources: ResourceSummary[]): string {
  ...
  const groupBlocks = [...grouped.entries()]
    .map(
      ([category, items]) => `<section class="mb-12">
        <h2 class="font-display text-3xl text-slate-900 mb-6">${escapeHtml(categoryLabel(category))}</h2>
  ...
  return htmlShell({
    ...
    description:
      'Free checklists, references and guides for SEO, accessibility, security, performance and AI readiness. Download as Markdown or PDF.',
...
export function renderResourceDetail(opts: RenderDetailOpts): string {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: resource.title,
        item: `${BASE_URL}/resources/${resource.slug}`,
      },
    ],
  };
  ...
  const body = `<main id="main-content">
    <article class="max-w-6xl mx-auto px-6 lg:px-12 py-12 lg:py-16">
      ...
      <header class="mb-10">
        <p class="inline-block px-2.5 py-1 text-xs font-semibold text-indigo-600 bg-indigo-50 rounded-md uppercase tracking-wider mb-4">${escapeHtml(categoryLabel(resource.category))}</p>
        <h1 class="font-display text-4xl lg:text-5xl text-slate-900 leading-tight mb-4">${escapeHtml(resource.title)}</h1>
        ${resource.subtitle ? `<p class="text-xl text-slate-600 leading-relaxed mb-2">${escapeHtml(resource.subtitle)}</p>` : ''}
        <p class="text-lg text-slate-700 leading-relaxed">${escapeHtml(resource.hook)}</p>
        ${resource.audience ? `<p class="mt-3 text-sm text-slate-600"><span class="font-semibold text-slate-700">For:</span> ${escapeHtml(resource.audience)}</p>` : ''}
      </header>
...
function renderGateForm(resource: GatedResource, typesetReady: boolean): string {
  const availableFormats = resource.formats.filter(
    (f) => !requiresTypeset(f) || typesetReady
  );
  const pendingFormats = resource.formats.filter(
    (f) => requiresTypeset(f) && !typesetReady
  );
    ...
    .map(
      (f) =>
        `<li class="flex items-center gap-2 text-sm text-slate-700">
          ...
          <span><strong class="font-semibold text-slate-900">${escapeHtml(FORMAT_LABELS[f])}</strong> · ${escapeHtml(FORMAT_DESCRIPTIONS[f])}</span>
        </li>`
    )
    .join('\n          ');
  ...
  return `<div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
    ...
    ${pendingHtml}
    <form method="POST" action="/resources/${escapeHtml(resource.slug)}/request" class="space-y-4 mt-5" novalidate>
      <div aria-hidden="true" style="position:absolute;left:-9999px;width:1px;height:1px;overflow:hidden;">
        <label for="gate-hp-website-${escapeHtml(resource.slug)}">Leave this field empty</label>
        <input type="text" id="gate-hp-website-${escapeHtml(resource.slug)}" name="website" tabindex="-1" autocomplete="off" />
      </div>
      <div>
    ...
    <p class="mt-5 pt-5 border-t border-slate-100 text-xs text-slate-600 text-center">
      Already have a Kritano account? <a href="/login?next=/resources/${escapeHtml(resource.slug)}" class="text-indigo-600 hover:text-indigo-700 font-medium">Sign in</a> to skip this step.
    </p>
  </div>`;
}
...
function renderLoggedInChooser(
  resource: GatedResource,
  ...
  const buttons = resource.formats
    .map((f) => {
      ...
      if (ready) {
        return `<a href="/api/resources/${escapeHtml(resource.slug)}/download/${escapeHtml(f)}" class="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-lg hover:border-indigo-300 hover:shadow-sm transition-all group">
          <div>
            <p class="font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">${escapeHtml(FORMAT_LABELS[f])}</p>
            <p class="text-xs text-slate-600 mt-0.5">${escapeHtml(FORMAT_DESCRIPTIONS[f])}</p>
          </div>
      ...
      return `<div class="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-lg">
          <div>
            <p class="font-semibold text-slate-600">${escapeHtml(FORMAT_LABELS[f])}</p>
...
export function renderResourceThanks(opts: RenderThanksOpts): string {
  ...
  const buttons = resource.formats
    .map((f) => {
      const ready = !requiresTypeset(f) || typesetReady;
      if (ready) {
        return `<a href="/api/resources/${escapeHtml(resource.slug)}/download/${escapeHtml(f)}?token=${encodeURIComponent(token)}" class="flex items-center justify-between p-5 bg-white border-2 border-indigo-200 rounded-lg hover:border-indigo-500 hover:bg-indigo-50 transition-all group">
          <div>
            <p class="font-display text-xl text-slate-900 group-hover:text-indigo-600 transition-colors">${escapeHtml(FORMAT_LABELS[f])}</p>
            <p class="text-sm text-slate-600 mt-1">${escapeHtml(FORMAT_DESCRIPTIONS[f])}</p>
          </div>
      ...
      return `<div class="flex items-center justify-between p-5 bg-slate-50 border-2 border-slate-200 rounded-lg">
          <div>
            <p class="font-display text-xl text-slate-600">${escapeHtml(FORMAT_LABELS[f])}</p>
  ...
  const body = `<main id="main-content">
    <div class="max-w-3xl mx-auto px-6 py-16 lg:py-24">
      ...
      <div class="mb-8">
        <p class="text-sm font-semibold text-slate-700 mb-3">${escapeHtml(resource.title)}</p>
        <div class="space-y-3">
          ${buttons}
        </div>
      </div>
  ...
  return htmlShell({
    ...
    canonicalUrl: `${BASE_URL}/resources/${resource.slug}/thanks`,
    ogImage: `${BASE_URL}/og-image.png`,
    ogType: 'website',
...
export function renderResourceNotFound(): string {
  const body = `<main id="main-content">
...
// ── Form-submission error pages (in-line redirects from POST handler) ─
...
export function renderFormError(opts: RenderFormErrorOpts): string {
  const { resource, errorMessage } = opts;
  const body = `<main id="main-content">
    <div class="max-w-2xl mx-auto px-6 py-24 text-center">
      <h1 class="font-display text-4xl text-slate-900 mb-4">${escapeHtml(errorMessage)}</h1>
      <p class="text-lg text-slate-600 mb-8">Please head back and try again.</p>
      <a href="/resources/${escapeHtml(resource.slug)}" class="inline-block px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition-colors">Back to ${escapeHtml(resource.title)}</a>
    </div>
  </main>`;
