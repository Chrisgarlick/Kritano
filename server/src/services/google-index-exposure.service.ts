...
export class GoogleIndexExposureService {
   ...
   */
  async scanDomain(domain: string): Promise<GoogleSearchItem[]> {
    ...
    if (r1?.items) allItems.push(...r1.items);
    if (r2?.items) allItems.push(...r2.items);
   ...
   */
  private async executeQuery(query: string, domain: string): Promise<GoogleSearchResponse | null> {
    try {
   ...
   */
  classifyResult(url: string, title: string): ClassifiedResult & { url: string } {
  ...
  convertToFindings(
    items: GoogleSearchItem[],
    robotsParser: RobotsParserService | null
  ): SecurityFinding[] {
...
export function createGoogleIndexExposureService(): GoogleIndexExposureService {
