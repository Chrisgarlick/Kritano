...
import { formatSeverity, truncate } from '../utils/formatting.js';

export function registerFindingTools(server: McpServer, pool: Pool, ctx: McpContext): void {

  // ── list_findings ────────────────────────────────────────────
  server.tool(
    'list_findings',
    'List findings (issues) for an audit. Filter by severity, category, WCAG criterion, or page URL. Results are ordered by severity (critical first).',
    {
      audit_id: z.string().uuid().describe('The audit job ID'),
      severity: z.enum(['critical', 'serious', 'moderate', 'minor', 'info']).optional().describe('Filter by severity'),
      category: z.enum(['seo', 'accessibility', 'security', 'performance', 'content', 'structured-data']).optional().describe('Filter by category'),
      wcag_criterion: z.string().optional().describe('Filter by WCAG criterion (e.g. "1.1.1")'),
      page_url: z.string().optional().describe('Filter findings to a specific page URL'),
      limit: z.number().int().min(1).max(100).optional().describe('Max results (default: 20)'),
      offset: z.number().int().min(0).optional().describe('Offset for pagination (default: 0)'),
    },
    async ({ audit_id, severity, category, wcag_criterion, page_url, limit, offset }) => {
      try {
        requireScope(ctx, 'findings:read');
        ...
        for (let i = 0; i < result.rows.length; i++) {
          ...
          lines.push(`${num}. ${formatSeverity(f.severity)} ${f.rule_name} (${f.category})`);
          if (f.wcag_criteria?.length) {
            lines.push(`   WCAG: ${f.wcag_criteria.join(', ')}`);
          }
          lines.push(`   ${truncate(f.message, 120)}`);
          if (f.page_url) {
  ...
  server.tool(
    'get_finding_detail',
    'Get full detail for a specific finding including the code snippet, recommendation, and help URL.',
    {
      finding_id: z.string().uuid().describe('The finding ID'),
    },
    async ({ finding_id }) => {
      try {
        requireScope(ctx, 'findings:read');
        ...
        const lines = [
          `${formatSeverity(f.severity)} ${f.rule_name}`,
          `Category: ${f.category}`,
          `Rule ID: ${f.rule_id}`,
        ];
  ...
  server.tool(
    'search_findings',
    'Search findings by keyword across rule names, messages, and descriptions for a specific audit.',
    {
      audit_id: z.string().uuid().describe('The audit job ID'),
      query: z.string().min(2).describe('Search keyword (e.g. "alt text", "heading", "contrast")'),
    ...
    async ({ audit_id, query, limit }) => {
      try {
        ...
        const result = await pool.query(
          `SELECT f.id, f.severity, f.rule_name, f.category, f.message, f.wcag_criteria, p.url as page_url
           FROM audit_findings f
           LEFT JOIN audit_pages p ON f.audit_page_id = p.id
           WHERE f.audit_job_id = $1
           ...
           ORDER BY
             CASE f.severity WHEN 'critical' THEN 1 WHEN 'serious' THEN 2 WHEN 'moderate' THEN 3 WHEN 'minor' THEN 4 ELSE 5 END
           LIMIT $3`,
          [audit_id, searchPattern, limitNum]
        );
  ...
  server.tool(
    'get_findings_summary',
    'Get an aggregated summary of findings for an audit -- counts by severity, category, top recurring rules, and most affected pages.',
    {
      audit_id: z.string().uuid().describe('The audit job ID'),
    },
    async ({ audit_id }) => {
      try {
        ...
        const auditCheck = await pool.query(
          `SELECT id, target_url FROM audit_jobs WHERE id = $1 AND user_id = $2`,
          [audit_id, ctx.userId]
        );
        if (auditCheck.rows.length === 0) {
          return { content: [{ type: 'text', text: `Audit "${audit_id}" not found.` }] };
        ...
        // Counts by severity
        const bySeverity = await pool.query<{ severity: string; count: string }>(
          `SELECT severity, COUNT(DISTINCT rule_id) as count FROM audit_findings WHERE audit_job_id = $1 GROUP BY severity`,
          [audit_id]
        );

        // Counts by category
        const byCategory = await pool.query<{ category: string; count: string }>(
          `SELECT category, COUNT(DISTINCT rule_id) as count FROM audit_findings WHERE audit_job_id = $1 GROUP BY category`,
          [audit_id]
        );

        // Top recurring rules
        const topRules = await pool.query<{ rule_id: string; rule_name: string; severity: string; category: string; page_count: string }>(
          `SELECT rule_id, rule_name, severity, category, COUNT(DISTINCT audit_page_id) as page_count
        ...
        // Most affected pages
        const topPages = await pool.query<{ url: string; finding_count: string }>(
          `SELECT p.url, COUNT(f.id) as finding_count
           FROM audit_findings f
        ...
        for (const row of topRules.rows) {
          lines.push(`  ${formatSeverity(row.severity)} ${row.rule_name} -- ${row.page_count} pages (${row.category})`);
        }
  ...
  server.tool(
    'get_wcag_coverage',
    'Show WCAG criterion coverage for an audit -- which WCAG criteria have issues and how many pages are affected.',
    {
      audit_id: z.string().uuid().describe('The audit job ID'),
    ...
    async ({ audit_id, level }) => {
      try {
        ...
        const auditCheck = await pool.query(
          `SELECT id, wcag_version, wcag_level FROM audit_jobs WHERE id = $1 AND user_id = $2`,
          [audit_id, ctx.userId]
        );
        if (auditCheck.rows.length === 0) {
          return { content: [{ type: 'text', text: `Audit "${audit_id}" not found.` }] };
        ...
        const result = await pool.query<{ criterion: string; severity: string; finding_count: string; page_count: string }>(
        ...
        for (const row of result.rows) {
          lines.push(`  ${row.criterion.padEnd(8)} ${formatSeverity(row.severity).padEnd(12)} ${row.finding_count} findings on ${row.page_count} pages`);
        }
