...
export function registerGscTools(server: McpServer, pool: Pool, ctx: McpContext): void {

  // ── get_gsc_overview ─────────────────────────────────────────
  server.tool(
    'get_gsc_overview',
    'Get Google Search Console overview for a connected property -- top queries, top pages, clicks, impressions, CTR, and position.',
    {
      site_id: z.string().uuid().describe('The site ID (must have GSC connected)'),
      period: z.enum(['7d', '28d', '90d']).optional().describe('Time period (default: 28d)'),
      limit: z.number().int().min(1).max(50).optional().describe('Max results per section (default: 10)'),
    },
    async ({ site_id, period, limit }) => {
      try {
        ...
        const siteCheck = await pool.query(
          `SELECT s.id, s.domain, g.id as gsc_id, g.property_url
           FROM sites s
  ...
  server.tool(
    'get_gsc_opportunities',
    'Find CTR opportunities -- queries where your position is good (top 10) but CTR is below average. These are quick wins for improving organic traffic.',
    {
    ...
    async ({ site_id, limit }) => {
      try {
        ...
        const siteCheck = await pool.query(
          `SELECT s.id, s.domain, g.id as gsc_id
           FROM sites s
           LEFT JOIN gsc_profiles g ON g.site_id = s.id
           WHERE s.id = $1 AND COALESCE(s.owner_id, s.created_by) = $2`,
          [site_id, ctx.userId]
        ...
        const result = await pool.query(
          `SELECT query, SUM(clicks) as clicks, SUM(impressions) as impressions,
           ...
           LIMIT $2`,
          [siteCheck.rows[0].gsc_id, limitNum]
        );
  ...
  server.tool(
    'get_gsc_cannibalisations',
    'Find keyword cannibalisation -- queries where multiple pages from your site compete for the same keyword.',
    {
      site_id: z.string().uuid().describe('The site ID'),
