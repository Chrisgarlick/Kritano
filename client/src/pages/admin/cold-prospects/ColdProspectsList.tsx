...
import {
  ...
  ChevronLeft, ChevronRight, Trash2, RotateCcw, ExternalLink, Filter,
} from 'lucide-react';
...
export default function ColdProspectsList() {
  ...
  }, [status, tld, hasEmail, hasName, search, page, sortBy, sortOrder, toast]);
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Link to="/admin/cold-prospects" className="text-slate-400 hover:text-white">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border transition-colors ${
              hasEmail ? 'bg-indigo-600/20 border-indigo-500 text-indigo-400' : 'bg-white/[0.02] border-white/[0.06] text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border transition-colors ${
              hasName ? 'bg-indigo-600/20 border-indigo-500 text-indigo-400' : 'bg-white/[0.02] border-white/[0.06] text-slate-400 hover:text-white'
            }`}
          >
          ...
          {(status || tld || search || hasEmail || hasName) && (
            <button
              onClick={() => setSearchParams(new URLSearchParams())}
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-slate-400 hover:text-white"
            >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.06] text-left">
                    ...
                    <th className="p-3 text-slate-400 font-medium cursor-pointer hover:text-white" onClick={() => handleSort('domain')}>
                      Domain<SortIcon col="domain" />
                    </th>
                    <th className="p-3 text-slate-400 font-medium">Status</th>
                    <th className="p-3 text-slate-400 font-medium cursor-pointer hover:text-white" onClick={() => handleSort('quality_score')}>
                      Score<SortIcon col="quality_score" />
                    </th>
                    <th className="p-3 text-slate-400 font-medium">Contact</th>
                    <th className="p-3 text-slate-400 font-medium">Tech</th>
                    <th className="p-3 text-slate-400 font-medium cursor-pointer hover:text-white" onClick={() => handleSort('created_at')}>
                      ...
                      <tr key={p.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                        ...
                        <td className="p-3">
                          <span className={`text-sm font-mono ${
                            ...
                            p.quality_score >= 30 ? 'text-amber-400' : 'text-slate-400'
                          }`}>
                            {p.quality_score}
                          </span>
                        </td>
                          ...
                          {p.contact_email ? (
                            <div>
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs text-white">{p.contact_email}</span>
                              </div>
                              {p.contact_name && (
                        ...
                        <td className="p-3">
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-indigo-400 rounded hover:bg-white/[0.06]"
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page <= 1}
                className="p-2 text-slate-400 hover:text-white disabled:opacity-30 rounded hover:bg-white/[0.06]"
              >
