...
3. **Add a simple deploy versioning mechanism.** Tag each deploy with `git rev-parse HEAD > /home/deploy/pagepulser/REVISION` and expose it via the `/health` endpoint. This makes it trivial to verify which version is running and to roll back to a specific commit.
